...
// v2.4 — zero lib dependencies. All logic inlined.
...
- DO NOT write fully transliterated sentences. Keep grammar English, use loanwords as nouns within English sentences.
...
- Goal: build a complete jewelry look, NOT 4 redundant items of the same category.
...
OUTPUT FORMAT:
Your ENTIRE response must be a single JSON object, starting with { and ending with }. No preamble. No code fences. No text before or after.
...
RATIONALE — avoid: "elevate", "timeless", "perfect for any occasion", "stunning", "gorgeous", "exquisite craftsmanship", "statement piece", "add a touch of". Be specific about visual or occasion link.

Pick exactly 4. Use only IDs from CANDIDATES list. Do not invent IDs.`;
...
function selectDiverseCandidates(target, allCandidates) {
  ...
  const sortByScore = (arr) => arr.map(c => ({ ...c, _score: scoreCandidate(target, c) })).sort((a, b) => b._score - a._score);
  const t1 = sortByScore(tier1), t2 = sortByScore(tier2), t3 = sortByScore(tier3), t4 = sortByScore(tier4);
  const composed = [...t1.slice(0, 12), ...t2.slice(0, 5), ...t3.slice(0, 3)];
  if (composed.length < 20) {
    const used = new Set(composed.map(c => c.shopify_product_id));
    const fillers = [...t1.slice(12), ...t2.slice(5), ...t3.slice(3), ...t4].filter(c => !used.has(c.shopify_product_id));
...
Max 2 [SAME-CAT], min 2 [CROSS-CAT]. Return only raw JSON object.`;
}
...
function extractJson(textAfterPrefill) {
  const withPrefill = '{' + textAfterPrefill;
  try { return JSON.parse(withPrefill); } catch {}
  const stripped = withPrefill.replace(/```\s*$/, '').trim();
  try { return JSON.parse(stripped); } catch {}
  const firstBrace = withPrefill.indexOf('{');
  const lastBrace = withPrefill.lastIndexOf('}');
  if (firstBrace !== -1 && lastBrace > firstBrace) {
    try { return JSON.parse(withPrefill.slice(firstBrace, lastBrace + 1)); } catch {}
  }
  throw new Error('Could not extract JSON');
}

// Generate picks for a single target product using a preloaded pool
async function generateForOne(supabase, client, target, pool) {
  ...
  // Enrich snapshots from the pool (no extra DB query)
  const poolMap = new Map(pool.map(p => [p.shopify_product_id, p]));
  const enrichedPicks = picks.map(p => {
    const poolRow = poolMap.get(p.shopify_product_id);
    return {
      ...p,
      title_snapshot: poolRow?.parent_title || null,
      image_snapshot: null, // image_url not in pool — lazy-enrich separately if needed
      price_snapshot: poolRow?.selling_price || null,
      category_snapshot: poolRow?.category || null,
    };
  });

  // Fetch image URLs for picks in one batch
...
export async function POST(request) {
  ...
  try {
    ...
    // Single Anthropic client reused across workers
