...
export async function POST(request) {
  ...
  if (action === 'update') {
    ...
    // Existing record — employee_id + date chahiye leave recalculation ke liye
    const { data: rec } = await supabase.from('employee_attendance')
      .select('employee_id, date').eq('id', id).single();
    const employee_id = rec?.employee_id;
    const date = rec?.date;

    const { data: emp } = employee_id
      ? await supabase.from('employees')
          .select('office_start, yearly_leaves_allowed, leaves_opening_used')
          .eq('id', employee_id).single()
      : { data: null };
    ...
    let late_minutes = 0;
    let final_status = status;
    // FIX (Jun 2026): leave_type ab status ke hisaab se RESET hota hai.
    // Pehle update leave_type ko haath hi nahi lagata tha — to jo din pehle
    // annual_leave tha aur baad mein present/late kar diya gaya, uska stale
    // 'annual_leave' stamp DB mein reh jaata tha aur salary usay leave gin leti
    // thi. Ab present/late/half_day pe leave_type=null, aur absent/leave pe
    // dobara annual-vs-unpaid compute hota hai.
    let leave_type = null;
    ...
    if (status === 'absent' || status === 'leave') {
      final_status = 'absent';
      const [dy, dm] = String(date || '').slice(0, 7).split('-').map(Number);
      const yearStart = dm >= 10 ? `${dy}-10-01` : `${dy - 1}-10-01`;
      const yearEnd   = dm >= 10 ? `${dy + 1}-09-30` : `${dy}-09-30`;
      const { count: usedLeaves } = await supabase
        .from('employee_attendance')
        ...
        .gte('date', yearStart)
        .lte('date', yearEnd)
        .neq('id', id);   // current record khud ko count na kare
      const allowed  = emp?.yearly_leaves_allowed || 14;
      const opening  = emp?.leaves_opening_used || 0;
      const remaining = allowed - ((usedLeaves || 0) + opening);
      leave_type = remaining > 0 ? 'annual_leave' : 'unpaid';
    }
