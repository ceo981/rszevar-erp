...
export async function GET(request) {
  ...
  // May 6 2026 — month-aware default. If no `upto` provided, use last day of
  // CURRENT month. (Old hardcoded fallback "2026-03-31" hata diya.)
  let upto = searchParams.get('upto');
  if (!upto) {
    const now = new Date();
    const lastDay = new Date(now.getFullYear(), now.getMonth() + 1, 0);
    upto = lastDay.toISOString().slice(0, 10);
  }
  ...
  const graceMinutes   = parseInt(pol.grace_minutes || 25);
  ...
  const leaderBonus1st = parseInt(pol.leaderboard_bonus_1st || 2000);
  const leaderBonus2nd = parseInt(pol.leaderboard_bonus_2nd || 1000);
  const lateChargeFirstHour = parseInt(pol.late_charge_first_hour || pol.late_deduction_amount || 100);
  const lateChargeSlab      = parseInt(pol.late_charge_per_30min_slab || 50);

  // Office time calculations
  const [oh, om] = officeStart.split(':').map(Number);
  const onTimeEndMins  = oh * 60 + om + graceMinutes;
  const onTimeEndHH    = Math.floor(onTimeEndMins / 60);
  const onTimeEndMM    = onTimeEndMins % 60;
  const onTimeEndStr   = `${String(onTimeEndHH).padStart(2,'0')}:${String(onTimeEndMM).padStart(2,'0')}`;
  // Free late period ends at 11:47 (just before charged late starts at 11:48)
  // We don't have a separate setting; derive from office_start + 47 mins (default convention)
  const freeLateEndMins = oh * 60 + om + 47;
  const freeLateEndStr  = `${String(Math.floor(freeLateEndMins / 60)).padStart(2,'0')}:${String(freeLateEndMins % 60).padStart(2,'0')}`;
  ...
  <div class="section">
    ...
    <div class="rules-grid">
      <div class="rule-box">
        ...
        <div class="rule-sub">Yahi time par check-in karna hai</div>
      </div>
      <div class="rule-box">
        <div class="rule-label">On-Time Window</div>
        <div class="rule-value">${officeStart} – ${onTimeEndStr}</div>
        <div class="rule-sub">${graceMinutes} min grace = ON TIME (no late count)</div>
      </div>
      <div class="rule-box">
        <div class="rule-label">Free Late Window</div>
        <div class="rule-value">${onTimeEndStr} – ${freeLateEndStr}</div>
        <div class="rule-sub">Late count hoga lekin free (within ${maxLates}/month)</div>
      </div>
      <div class="rule-box">
        <div class="rule-label">Free Lates / Month</div>
        <div class="rule-value">${maxLates}</div>
        <div class="rule-sub">Inse zyada = charged late shuru</div>
      </div>
      <div class="rule-box">
        <div class="rule-label">Free Half Days / Month</div>
        <div class="rule-value">${maxHalfDays}</div>
        <div class="rule-sub">Sirf ${maxHalfDays} half-day free</div>
      </div>
      <div class="rule-box">
        <div class="rule-label">Charged Late Start</div>
        <div class="rule-value">After ${maxLates} lates</div>
        <div class="rule-sub">Quota exceed hone par charge applicable</div>
      </div>
      <div class="rule-box">
        <div class="rule-label">First Hour Charge</div>
        <div class="rule-value">Rs. ${lateChargeFirstHour}</div>
        <div class="rule-sub">${officeStart} – 12:00 PM arrival pe</div>
      </div>
      <div class="rule-box">
        <div class="rule-label">Per 30min Slab</div>
        <div class="rule-value">+Rs. ${lateChargeSlab}</div>
        <div class="rule-sub">12:00 PM ke baad har 30 min slab</div>
      </div>
      <div class="rule-box">
    ...
    <!-- Late Charge Tier Table (May 6 2026) -->
    <div style="margin-top:18px;background:#fef3c7;border:1px solid #fcd34d;border-radius:8px;padding:14px;">
      <div style="font-size:13px;color:#92400e;font-weight:700;margin-bottom:10px;text-transform:uppercase;letter-spacing:0.5px;">
        💰 Charged Late Tier Table (after ${maxLates} free lates exceed)
      </div>
      <table style="width:100%;border-collapse:collapse;font-size:13px;">
        <thead>
          <tr style="background:#fde68a;">
            <th style="text-align:left;padding:6px 10px;border:1px solid #fbbf24;">Arrival Time</th>
            <th style="text-align:right;padding:6px 10px;border:1px solid #fbbf24;">Cut Amount</th>
          </tr>
        </thead>
        <tbody>
          <tr><td style="padding:5px 10px;border:1px solid #fde68a;">${officeStart} – 12:00 PM</td><td style="text-align:right;padding:5px 10px;border:1px solid #fde68a;font-weight:600;">Rs. ${lateChargeFirstHour}</td></tr>
          <tr><td style="padding:5px 10px;border:1px solid #fde68a;">12:01 – 12:30 PM</td><td style="text-align:right;padding:5px 10px;border:1px solid #fde68a;font-weight:600;">Rs. ${lateChargeFirstHour + lateChargeSlab}</td></tr>
          <tr><td style="padding:5px 10px;border:1px solid #fde68a;">12:31 – 1:00 PM</td><td style="text-align:right;padding:5px 10px;border:1px solid #fde68a;font-weight:600;">Rs. ${lateChargeFirstHour + 2 * lateChargeSlab}</td></tr>
          <tr><td style="padding:5px 10px;border:1px solid #fde68a;">1:01 – 1:30 PM</td><td style="text-align:right;padding:5px 10px;border:1px solid #fde68a;font-weight:600;">Rs. ${lateChargeFirstHour + 3 * lateChargeSlab}</td></tr>
          <tr><td style="padding:5px 10px;border:1px solid #fde68a;">1:31 – 2:00 PM</td><td style="text-align:right;padding:5px 10px;border:1px solid #fde68a;font-weight:600;">Rs. ${lateChargeFirstHour + 4 * lateChargeSlab}</td></tr>
          <tr><td style="padding:5px 10px;border:1px solid #fde68a;font-style:italic;color:#666;">… (har 30 min mein +Rs. ${lateChargeSlab})</td><td style="text-align:right;padding:5px 10px;border:1px solid #fde68a;font-style:italic;color:#666;">+Rs. ${lateChargeSlab} per slab</td></tr>
        </tbody>
      </table>
    </div>

    <div class="rule-desc">
      <b>Late ka Rule:</b> ${officeStart} se ${graceMinutes} min ke andar (yaani ${onTimeEndStr} tak) check-in = <b>ON TIME</b>.
      ${onTimeEndStr} ke baad <b>Late</b> count hoga. Mahine mein <b>${maxLates} Late + ${maxHalfDays} Half Day</b> bilkul free hain — salary nahi kategi.
      <br><br>
      <b>Charged Late:</b> ${maxLates} free quota exceed hone ke baad har late ka charge actual arrival time pe depend karta hai (upar wala table dekhein).
      System pehle ${maxLates} lates ko free count karta hai (date order mein), uske baad wali charged hoti hain.
      <br><br>
      <b>Time Bonus:</b> Milega SIRF tab jab: Lates <b>${maxLates} ya kam</b> hon AUR Half days <b>${maxHalfDays} ya kam</b> hon AUR koi paid absent NA ho. Koi bhi rule tuta = Time Bonus nahi milega.
    </div>
  </div>
