...
export async function POST(request) {
  ...
  try {
    ...
    // ── Step 5: Product metafields (Occasion, Set Contents, Stone Type, Material, Color) ──
    // Overrides from modal (if user edited in UI) take precedence over AI output.
    const metafieldMap = [
      {
        push_key: 'mf_occasion',
        mf_key: 'occasion',
        type: 'list.single_line_text_field',
        value: fields_to_push.mf_occasion_override !== undefined
          ? fields_to_push.mf_occasion_override
          : g.occasions_list,
      },
      {
        push_key: 'mf_set_contents',
        mf_key: 'set_contents',
        type: 'list.single_line_text_field',
        value: fields_to_push.mf_set_contents_override !== undefined
          ? fields_to_push.mf_set_contents_override
          : g.set_contents_list,
      },
      {
        push_key: 'mf_stone_type',
        mf_key: 'stone_type',
        type: 'list.single_line_text_field',
        value: fields_to_push.mf_stone_type_override !== undefined
          ? fields_to_push.mf_stone_type_override
          : g.stone_types_list,
      },
      {
        push_key: 'mf_material',
        mf_key: 'material',
        type: 'single_line_text_field',
        value: fields_to_push.mf_material_override !== undefined
          ? fields_to_push.mf_material_override
          : g.material,
      },
      {
        push_key: 'mf_color_finish',
        mf_key: 'color_finish',
        type: 'single_line_text_field',
        value: fields_to_push.mf_color_finish_override !== undefined
          ? fields_to_push.mf_color_finish_override
          : g.color_finish,
      },
    ];

    for (const mf of metafieldMap) {
      // Skip if user unchecked this metafield OR AI didn't produce a value
      if (!fields_to_push[mf.push_key]) continue;
      const isList = mf.type === 'list.single_line_text_field';
      if (isList) {
        if (!Array.isArray(mf.value) || mf.value.length === 0) continue;
      } else {
        if (!mf.value || typeof mf.value !== 'string' || !mf.value.trim()) continue;
      }

      try {
        const stringValue = isList ? JSON.stringify(mf.value) : String(mf.value).trim();
        await upsertMetafield(productId, 'rszevar', mf.mf_key, stringValue, mf.type);
        results[mf.push_key] = { success: true, value: mf.value };
        // Rate limit: Shopify allows 2 req/sec on REST; brief pause
        await new Promise(r => setTimeout(r, 120));
      } catch (e) {
        errors[mf.push_key] = e.message;
      }
    }

    // ── Step 6: Mark enhancement as pushed ──
