...
═══ METAFIELDS (5 structured fields for filtering + smart collections) ═══
Infer these from product title, current description, and user-provided facts. Be accurate — omit a value rather than guess. All values must come from approved vocabularies below. These power smart collections (auto-populated) and product filters.

occasions_list (1-4 items, use user's target_occasions as primary signal):
  Approved values: Bridal, Mehndi, Nikkah, Walima, Engagement, Party, Eid, Daily wear, Formal, Gifting
  A single piece can fit multiple occasions.

set_contents_list (1-6 items, only what's actually in the product):
  Approved values: Necklace, Earrings, Jhumka, Maang Tikka, Tikka, Nath, Ring, Ranihaar, Bangles, Bracelet, Pendant, Choker, Anklet, Hair accessory
  Rule: "12 Pcs Bangles Set" = ["Bangles"]. "Bridal Set" typically = ["Necklace", "Earrings", "Maang Tikka"]. If a pair of earrings only, use ["Earrings"].

stone_types_list (1-3 items):
  Approved values: Kundan, Polki, Zircon, AD stones, Meenakari, Pearl, Ruby, Emerald, Turkish stones, Enamel work, Stone-free
  If no stones visible, use ["Stone-free"]. Multiple allowed (e.g., ["Kundan", "Pearl"]).

material (single string, under 60 chars):
  Format: "<plating/base> [finish]"
  Examples: "Gold-plated brass", "Silver-plated alloy", "Oxidized brass with enamel", "24k gold-plated with antique finish"
  Be specific when user facts provide it; otherwise default to "Gold-plated brass" for RS ZEVAR products.

color_finish (single string, under 60 chars):
  Format: "<primary color> [+ secondary] [finish]"
  Examples: "Antique gold with green highlights", "Rose gold with white pearls", "Classic gold", "Silver with blue enamel"
  Describe the dominant visual impression.

═══ UPDATED OUTPUT FORMAT (append 5 new fields to earlier schema) ═══
The full JSON object you must return is:

{
  ...
  "seo_score": 85,
  "seo_notes": "...",
  "occasions_list": ["Bridal", "Mehndi"],
  "set_contents_list": ["Necklace", "Earrings", "Maang Tikka"],
  "stone_types_list": ["Kundan", "Polki"],
  "material": "Gold-plated brass with enamel",
  "color_finish": "Antique gold with green highlights"
}
