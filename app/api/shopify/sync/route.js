...
import { fetchAllOrdersSince, transformOrder, transformLineItems, isActiveLineItem, buildImageLookupMaps } from '../../../../lib/shopify';
...
export async function POST(request) {
  ...
  try {
    ...
    // Pre-fetch image lookup maps from products table.
    // FIX May 2026 — Build BOTH sku and variant_id maps (variant_id wins for
    // same-SKU variants). See lib/shopify.js#buildImageLookupMaps.
    ...
    if (newOrdersMap.size > 0) {
      ...
      for (const shopifyOrder of shopifyOrders) {
        ...
        if (!dbId) continue;
        const items = transformLineItems(shopifyOrder, skuImageMap, variantImageMap).map(i => ({ ...i, order_id: dbId }));
        allItems.push(...items);
      }
