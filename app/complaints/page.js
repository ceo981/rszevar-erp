...
function AddModal({ onClose, onSave }) {
  ...
  const save = async () => {
    ...
    setSaving(true);
    const r = await fetch('/api/complaints', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'add', ...form }) });
  ...
  const inp = (label, key, type = 'text', opts = {}) => (
    <div>
      ...
      {opts.select ? (
        <select value={form[key]} onChange={e => setForm(f => ({...f, [key]: e.target.value}))} style={{ width: '100%', background: '#1a1a1a', border: `1px solid ${border}`, color: '#fff', borderRadius: 7, padding: '9px 12px', fontSize: 13, fontFamily: 'inherit' }}>
    ...
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.8)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <div style={{ background: '#0f0f0f', border: `1px solid ${border}`, borderRadius: 12, padding: 28, width: 460, maxHeight: '90vh', overflowY: 'auto' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 13 }}>
          ...
          <button onClick={save} disabled={saving} style={{ background: '#ef4444', color: '#fff', border: 'none', borderRadius: 8, padding: '11px', fontWeight: 700, fontSize: 14, cursor: 'pointer' }}>
...
function ComplaintPanel({ complaint, onClose, onSave }) {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 1000, display: 'flex' }}>
      ...
      <div style={{ width: 440, background: '#0f0f0f', borderLeft: `1px solid ${border}`, overflowY: 'auto' }}>
        ...
        <div style={{ padding: '20px 24px' }}>
          {/* Description */}
          <div style={{ background: card, border: `1px solid ${border}`, borderRadius: 8, padding: '14px', marginBottom: 20 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            <div>
              ...
              <select value={form.status} onChange={e => setForm(f => ({...f, status: e.target.value}))} style={{ width: '100%', background: '#1a1a1a', border: `1px solid ${border}`, color: '#fff', borderRadius: 7, padding: '9px 12px', fontSize: 13, fontFamily: 'inherit' }}>
            ...
            <div>
              ...
              <select value={form.priority} onChange={e => setForm(f => ({...f, priority: e.target.value}))} style={{ width: '100%', background: '#1a1a1a', border: `1px solid ${border}`, color: '#fff', borderRadius: 7, padding: '9px 12px', fontSize: 13, fontFamily: 'inherit' }}>
            ...
            <div>
              ...
              <select value={form.assigned_to} onChange={e => setForm(f => ({...f, assigned_to: e.target.value}))} style={{ width: '100%', background: '#1a1a1a', border: `1px solid ${border}`, color: '#fff', borderRadius: 7, padding: '9px 12px', fontSize: 13, fontFamily: 'inherit' }}>
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              <button onClick={update} disabled={saving} style={{ flex: 1, background: gold, color: '#000', border: 'none', borderRadius: 8, padding: '10px', fontWeight: 700, fontSize: 13, cursor: 'pointer' }}>
                {saving ? '...' : '✓ Update'}
              </button>
    ...
    <div style={{ fontFamily: 'Inter, sans-serif', color: '#fff', padding: 24 }}>
        ...
        {['all', 'open', 'in_progress', 'resolved', 'closed'].map(s => {
          ...
          return (
            <button key={s} onClick={() => setStatusFilter(s)}
              style={{ padding: '6px 14px', background: statusFilter === s ? '#1e1e1e' : 'transparent', border: `1px solid ${statusFilter === s ? sc.color+'44' : border}`, borderRadius: 7, fontSize: 12, color: statusFilter === s ? sc.color : '#555', cursor: 'pointer', fontFamily: 'inherit' }}>
              {sc.label}
            </button>
          );
        })}
        <select value={categoryFilter} onChange={e => setCategoryFilter(e.target.value)} style={{ background: '#1a1a1a', border: `1px solid ${border}`, color: '#888', borderRadius: 7, padding: '6px 12px', fontSize: 12, fontFamily: 'inherit' }}>
          <option value="all">All Categories</option>
          {CATEGORIES.map(c => <option key={c} value={c}>{c}</option>)}
        </select>
        <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search..." style={{ background: card, border: `1px solid ${border}`, color: '#fff', borderRadius: 7, padding: '6px 12px', fontSize: 12, minWidth: 160 }} />
      </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
        ...
        {complaints.map((c, i) => {
          ...
          return (
            <div key={i} onClick={() => setSelected(c)} style={{ background: card, border: `1px solid ${c.status === 'open' ? '#33000088' : border}`, borderRadius: 10, padding: '16px 18px', cursor: 'pointer', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 16 }}>
