...
function UploadSection({ onDone }) {
  ...
  const handlePreview = async () => {
    ...
    try {
      ...
      if (d.success) setPreview(d); else setError(d.error || 'Parse error');
      ...
      {result && (
        <div style={{ background: '#001a0a', border: '1px solid #003300', borderRadius: 12, padding: 20, marginBottom: 24 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 12, marginBottom: 16 }}>
            {[['💰 Paid Marked', result.orders_paid, '#22c55e'], ['↩️ RTO Marked', result.orders_rto, '#ef4444'], ['⏭ Already Paid', result.already_paid, '#888'], ['🔍 Not Found', result.not_found, '#f59e0b'], ['⏩ Skipped', result.skipped, '#555'], ['📋 Total Parsed', result.total_parsed, gold]].map(([label, val, color]) => (
          ...
          <button onClick={reset} style={{ marginTop: 14, background: '#1a1a1a', border: `1px solid ${border}`, color: '#888', borderRadius: 7, padding: '7px 16px', fontSize: 12, cursor: 'pointer', fontFamily: 'inherit' }}>🔄 Nai File Upload Karo</button>
        </div>
      )}

      {!result && (
        <div style={{ background: card, border: `1px solid ${border}`, borderRadius: 12, padding: 24 }}>
          <div style={{ fontSize: 14, fontWeight: 700, color: gold, marginBottom: 20 }}>📁 Settlement File Upload</div>
          <div style={{ marginBottom: 16 }}>
            <div style={{ fontSize: 11, color: '#555', marginBottom: 8, textTransform: 'uppercase', letterSpacing: 0.5 }}>Courier</div>
            <div style={{ display: 'flex', gap: 8 }}>
              {COURIERS.map(c => (
                <button key={c.id} onClick={() => { setCourier(c.id); reset(); }} style={{ flex: 1, padding: '10px', borderRadius: 8, cursor: 'pointer', fontFamily: 'inherit', fontSize: 13, fontWeight: 600, background: courier === c.id ? gold + '22' : '#0a0a0a', border: `1px solid ${courier === c.id ? gold : border}`, color: courier === c.id ? gold : '#555' }}>
                  {c.label}
          ...
          <div style={{ marginBottom: 16 }}>
            ...
            <input type="file" accept={selectedCourier?.accept} onChange={e => { setFile(e.target.files[0] || null); setPreview(null); setError(''); }} style={{ ...inp, width: '100%', cursor: 'pointer' }} />
          ...
          {error && <div style={{ background: '#1a0000', border: '1px solid #330000', borderRadius: 8, padding: '10px 14px', fontSize: 13, color: '#ef4444', marginBottom: 16 }}>❌ {error}</div>}
          {preview && (
            <div style={{ background: '#0a0a0a', border: `1px solid ${border}`, borderRadius: 10, padding: 16, marginBottom: 16 }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 10, marginBottom: 12 }}>
                {[['💰 Paid Markenge', preview.to_mark_paid, '#22c55e'], ['↩️ RTO Markenge', preview.to_mark_rto, '#ef4444'], ['⏭ Already Paid', preview.already_paid, '#888'], ['🔍 Not Found', preview.not_found, '#f59e0b'], ['⏩ Skip (0-amt)', preview.skipped, '#555'], ['📋 Total', preview.total_parsed, gold]].map(([label, val, color]) => (
          ...
          <div style={{ display: 'flex', gap: 10 }}>
            {!preview ? (
              <button onClick={handlePreview} disabled={!file || loading} style={{ flex: 1, background: file ? gold + '22' : '#0a0a0a', border: `1px solid ${file ? gold : border}`, color: file ? gold : '#555', borderRadius: 8, padding: '11px', fontSize: 14, fontWeight: 700, cursor: file && !loading ? 'pointer' : 'not-allowed', fontFamily: 'inherit' }}>
                {loading ? '⟳ Parsing...' : '🔍 Preview'}
              </button>
            ) : (
              <>
                <button onClick={() => setPreview(null)} disabled={loading} style={{ flex: 1, background: '#0a0a0a', border: `1px solid ${border}`, color: '#888', borderRadius: 8, padding: '11px', fontSize: 13, cursor: 'pointer', fontFamily: 'inherit' }}>✕ Wapas</button>
                <button onClick={handleApply} disabled={loading} style={{ flex: 2, background: '#22c55e22', border: '1px solid #22c55e44', color: '#22c55e', borderRadius: 8, padding: '11px', fontSize: 14, fontWeight: 700, cursor: loading ? 'not-allowed' : 'pointer', fontFamily: 'inherit' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14 }}>
        ...
        <div style={{ display: 'flex', gap: 6 }}>
          {['All', 'Leopards', 'Kangaroo', 'PostEx'].map(c => (
            <button key={c} onClick={() => setFilter(c)} style={{ background: filter === c ? '#1e1e1e' : 'transparent', border: `1px solid ${filter === c ? '#2a2a2a' : 'transparent'}`, color: filter === c ? gold : '#555', borderRadius: 6, padding: '5px 12px', fontSize: 12, cursor: 'pointer', fontFamily: 'inherit' }}>{c}</button>
          ))}
          <button onClick={load} style={{ background: 'transparent', border: `1px solid ${border}`, color: '#555', borderRadius: 6, padding: '5px 10px', fontSize: 12, cursor: 'pointer' }}>⟳</button>
        </div>
      </div>
      <div style={{ background: card, border: `1px solid ${border}`, borderRadius: 10, overflow: 'hidden' }}>
        {loading ? (
          <div style={{ padding: 30, textAlign: 'center', color: '#444', fontSize: 13 }}>Loading...</div>
        ) : filtered.length === 0 ? (
          <div style={{ padding: 30, textAlign: 'center', color: '#333', fontSize: 13 }}>Koi record nahi — pehla settlement upload karo</div>
        ) : (
            ...
            <tbody>
              {filtered.map((s, i) => (
                <tr key={s.id} style={{ borderBottom: `1px solid #111`, background: i % 2 === 0 ? 'transparent' : '#0a0a0a' }}>
                  ...
                  <td style={{ padding: '10px 14px' }}><span style={{ color: courierColor[s.courier] || '#888', fontWeight: 600, fontSize: 12 }}>{s.courier}</span></td>
