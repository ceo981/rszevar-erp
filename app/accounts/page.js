...
// ── Shared style atoms ──────────────────────────────────────────────────────
const S = {
  input:   { width: '100%', background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 8, padding: '10px 12px', color: 'var(--text)', fontSize: 13, outline: 'none', boxSizing: 'border-box', fontFamily: 'inherit' },
  btn:     { background: 'var(--bg-section)', border: '1px solid var(--border)', borderRadius: 8, padding: '8px 16px', color: 'var(--gold)', fontSize: 13, cursor: 'pointer', fontFamily: 'inherit', fontWeight: 600 },
  label:   { display: 'block', fontSize: 11, color: 'var(--text3)', marginBottom: 6, fontFamily: 'monospace', letterSpacing: 0.5, textTransform: 'uppercase' },
  td:      { padding: '12px 16px', fontSize: 13, color: 'var(--text2)', verticalAlign: 'middle' },
  th:      { padding: '10px 16px', textAlign: 'left', fontSize: 11, color: 'var(--text3)', fontFamily: 'monospace', letterSpacing: 1, textTransform: 'uppercase', fontWeight: 400, borderBottom: '1px solid var(--border)' },
  card:    { background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 12, padding: '20px 24px' },
  section: { background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 12, overflow: 'hidden', marginBottom: 20 },
};
...
function Modal({ title, onClose, children, width = 500 }) {
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.8)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
      <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 14, padding: 24, width: '100%', maxWidth: width, maxHeight: '90vh', overflowY: 'auto' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
          <div style={{ fontSize: 16, fontWeight: 700, color: 'var(--gold)' }}>{title}</div>
          <button onClick={onClose} style={{ background: 'none', border: 'none', color: 'var(--text3)', fontSize: 22, cursor: 'pointer', padding: 0, width: 36, height: 36, display: 'flex', alignItems: 'center', justifyContent: 'center', borderRadius: 8 }}>✕</button>
        </div>
        {children}
      </div>
    </div>
  ...
  return (
    <div style={{ ...S.card, borderColor: border || '#1e1e1e' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 8 }}>
        <span style={{ fontSize: 11, color: 'var(--text3)', fontFamily: 'monospace', letterSpacing: 1, textTransform: 'uppercase' }}>{label}</span>
...
function DashboardTab() {
  ...
  if (loading) return <div style={{ padding: 60, textAlign: 'center', color: 'var(--text3)' }}>Loading financial data...</div>;
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
      ...
      <div style={{ background: pl.net >= 0 ? 'rgba(34,197,94,0.05)' : 'rgba(239,68,68,0.05)', border: `1px solid ${pl.net >= 0 ? '#22c55e33' : '#ef444433'}`, borderRadius: 14, padding: 24 }}>
        <div style={{ fontSize: 12, color: 'var(--text3)', fontFamily: 'monospace', letterSpacing: 1, textTransform: 'uppercase', marginBottom: 12 }}>📊 Profit & Loss — {month}</div>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: 20 }}>
          <div><div style={{ fontSize: 11, color: 'var(--text3)', marginBottom: 4 }}>Revenue (Delivered)</div><div style={{ fontSize: 28, fontWeight: 700, color: '#22c55e' }}>{fmt(pl.revenue)}</div></div>
          <div><div style={{ fontSize: 11, color: 'var(--text3)', marginBottom: 4 }}>Total Expenses</div><div style={{ fontSize: 28, fontWeight: 700, color: '#ef4444' }}>{fmt(pl.total_expenses)}</div></div>
          <div><div style={{ fontSize: 11, color: 'var(--text3)', marginBottom: 4 }}>Net {pl.net >= 0 ? 'Profit' : 'Loss'}</div><div style={{ fontSize: 32, fontWeight: 800, color: plColor, letterSpacing: -1 }}>{fmt(Math.abs(pl.net))}</div><div style={{ fontSize: 12, color: plColor, marginTop: 2 }}>{pl.margin}% margin</div></div>
        </div>
      </div>

      {/* Orders */}
      <div>
        <div style={{ fontSize: 12, color: 'var(--text3)', fontFamily: 'monospace', letterSpacing: 1, textTransform: 'uppercase', marginBottom: 12 }}>📋 Orders (All Time)</div>
      ...
      {/* Courier breakdown */}
      <div>
        <div style={{ fontSize: 12, color: 'var(--text3)', fontFamily: 'monospace', letterSpacing: 1, textTransform: 'uppercase', marginBottom: 12 }}>🚚 Courier Breakdown</div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                  {[['Orders', co.orders || 0, '#ccc'], ['Delivered', co.delivered || 0, '#22c55e'], ['Revenue', fmt(co.revenue || 0), '#ccc'], ['Settled', fmt(settled), '#22c55e']].map(([l, v, col]) => (
                    <div key={l} style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12 }}>
                      <span style={{ color: 'var(--text3)' }}>{l}</span><span style={{ color: col, fontWeight: l === 'Settled' ? 700 : 400 }}>{v}</span>
                    </div>
                  ))}
                </div>
              </div>
      ...
      {/* Expenses */}
      <div>
        <div style={{ fontSize: 12, color: 'var(--text3)', fontFamily: 'monospace', letterSpacing: 1, textTransform: 'uppercase', marginBottom: 12 }}>💸 Expenses Breakdown</div>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: 10 }}>
        ...
        {Object.keys(expenses.by_category || {}).length > 0 && (
          <div style={{ marginTop: 10, display: 'flex', gap: 8, flexWrap: 'wrap' }}>
            {Object.entries(expenses.by_category).map(([cat, amt]) => (
              <span key={cat} style={{ background: 'var(--bg-section)', border: '1px solid var(--border)', borderRadius: 20, padding: '4px 12px', fontSize: 12, color: 'var(--text3)' }}>{cat}: <span style={{ color: '#ef4444' }}>{fmt(amt)}</span></span>
            ))}
          </div>
        )}
      </div>
      ...
      {/* Settlements */}
      <div>
        <div style={{ fontSize: 12, color: 'var(--text3)', fontFamily: 'monospace', letterSpacing: 1, textTransform: 'uppercase', marginBottom: 12 }}>✅ Settlements Received</div>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: 10 }}>
      ...
      {/* Capital */}
      <div>
        <div style={{ fontSize: 12, color: 'var(--text3)', fontFamily: 'monospace', letterSpacing: 1, textTransform: 'uppercase', marginBottom: 12 }}>🏦 Capital & Liabilities</div>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: 10 }}>
...
function VendorsTab() {
  ...
  const ListPanel = (
    <div style={{ width: isMobile ? '100%' : 320, flexShrink: 0 }}>
      ...
      {totalOut > 0 && (
        <div style={{ background: 'linear-gradient(135deg, var(--red-dim) 0%, #1f1414 100%)', border: '1px solid #ef444433', borderRadius: 12, padding: '14px 18px', marginBottom: 12, boxShadow: '0 2px 12px rgba(239,68,68,0.08)' }}>
      ...
      {canVendorEdit && (
        <button onClick={() => setShowAdd(true)} style={{ ...S.btn, width: '100%', marginBottom: 10, padding: '10px 16px', background: 'var(--gold)', color: 'var(--text)', borderColor: '#c9a96e' }}>
          + Add New Vendor
        </button>
      )}
      <input
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
        {visibleLedger.length === 0 && (
          <div style={{ padding: 32, textAlign: 'center', color: 'var(--text3)', fontSize: 13, background: 'var(--bg)', border: '1px dashed var(--border)', borderRadius: 10 }}>
            <div style={{ fontSize: 28, marginBottom: 8, opacity: 0.6 }}>🏭</div>
            <div style={{ color: 'var(--text2)', marginBottom: 4 }}>{search ? 'No vendors match your search' : 'No vendors yet'}</div>
            <div style={{ fontSize: 11, color: 'var(--text3)' }}>{search ? 'Try a different keyword' : 'Add your first vendor to get started'}</div>
          </div>
        )}
        {visibleLedger.map(v => {
            ...
            >
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap' }}>
                  <span style={{ fontSize: 10, color: 'var(--text3)', background: 'var(--bg-card)', padding: '1px 7px', borderRadius: 10, border: '1px solid var(--border)' }}>{v.category || 'General'}</span>
                  {v.outstanding > 0 ? (
              ...
              {isMobile && <span style={{ color: 'var(--text3)', fontSize: 16 }}>›</span>}
            </button>
          );
        })}
      </div>
  ...
  const DetailPanel = (
    <div style={{ flex: 1, minWidth: 0 }}>
      {!selected ? (
        <div style={{ padding: '60px 24px', textAlign: 'center', color: 'var(--text3)', background: 'var(--bg)', border: '1px dashed var(--border)', borderRadius: 12 }}>
          <div style={{ fontSize: 36, marginBottom: 10, opacity: 0.4 }}>👈</div>
          <div style={{ fontSize: 14, color: 'var(--text3)' }}>Select a vendor to view their ledger</div>
        </div>
      ) : (
        <>
          {/* Header card */}
            ...
            <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12, flexWrap: 'wrap' }}>
              ...
              {isMobile && (
                <button
                  ...
                  style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', color: 'var(--text2)', borderRadius: 8, padding: '6px 12px', fontSize: 12, cursor: 'pointer', fontFamily: 'inherit', flexShrink: 0 }}
                >← Back</button>
              )}
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
                  ...
                  {selV && canVendorEdit && (
                    <button
                      ...
                      style={{ background: 'transparent', border: '1px solid var(--border)', color: 'var(--gold)', cursor: 'pointer', fontSize: 11, padding: '3px 9px', borderRadius: 6, fontFamily: 'inherit', fontWeight: 500 }}
                    >✏️ Edit</button>
                  )}
                </div>
                <div style={{ fontSize: 12, color: 'var(--text3)', marginTop: 5, display: 'flex', flexWrap: 'wrap', gap: 8 }}>
                  ...
                  {selV?.category        && <span style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 4, padding: '1px 7px', fontSize: 10 }}>{selV.category}</span>}
                </div>
              </div>
            </div>
            ...
            {selData && (
              <div style={{
                ...
                {[
                  ...
                  <div key={s.l}>
                    <div style={{ fontSize: 10, color: 'var(--text3)', fontFamily: 'monospace', letterSpacing: 0.5, textTransform: 'uppercase' }}>{s.l}</div>
      ...
      {showAdd && (
        <Modal title={editingVendor ? '✏️ Edit Vendor' : '🏭 New Vendor'} onClose={closeAdd}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            ...
            <button onClick={addVendor} disabled={saving} style={{ ...S.btn, background: 'var(--gold)', color: 'var(--text)', fontWeight: 700, padding: '11px' }}>
  ...
  return (
    <div style={S.section}>
      <table style={{ width: '100%', borderCollapse: 'collapse' }}>
        ...
        <tbody>
          {!transactions.length && (
            <tr><td colSpan={6} style={{ padding: 40, textAlign: 'center', color: 'var(--text3)' }}>No transactions yet</td></tr>
          )}
          {transactions.map(t => {
            const isP = t.payment_type === 'purchase';
            ...
            return (
              <tr key={t.id} style={{ borderBottom: '1px solid #1a1a1a' }}>
                ...
                <td style={{ ...S.td, color: 'var(--text2)' }}>{t.item_description || t.note || '—'}</td>
                ...
                <td style={S.td}>
                  <div style={{ display: 'flex', gap: 6, justifyContent: 'flex-end' }}>
                    {canEdit && <button onClick={() => onEdit(t)} title="Edit entry" style={{ background: 'var(--bg-section)', border: '1px solid var(--border)', color: 'var(--gold)', cursor: 'pointer', fontSize: 12, padding: '5px 10px', borderRadius: 6, fontFamily: 'inherit' }}>✏️ Edit</button>}
...
function TxnCardList({ transactions, canEdit, onEdit, onDelete }) {
  if (!transactions.length) {
    return (
      <div style={{ padding: 32, textAlign: 'center', color: 'var(--text3)', background: 'var(--bg)', border: '1px dashed var(--border)', borderRadius: 10 }}>
        <div style={{ fontSize: 28, marginBottom: 8, opacity: 0.5 }}>📒</div>
        <div style={{ fontSize: 13, color: 'var(--text2)' }}>No transactions yet</div>
        <div style={{ fontSize: 11, color: 'var(--text3)', marginTop: 4 }}>Add a purchase or payment to start the ledger</div>
      </div>
    );
  }
  return (
          ...
          >
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 8, gap: 8 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
                ...
                <span style={{ fontSize: 11, color: 'var(--text3)' }}>{fmtDate(t.payment_date)}</span>
              </div>
            ...
            {(t.item_description || t.note) && (
              <div style={{ fontSize: 13, color: 'var(--text2)', marginBottom: 8, lineHeight: 1.4, wordBreak: 'break-word' }}>
                {t.item_description || t.note}
              </div>
            )}
            ...
            {canEdit && (
              <div style={{ display: 'flex', gap: 8, marginTop: 8, paddingTop: 8, borderTop: '1px solid #1a1a1a' }}>
                <button onClick={() => onEdit(t)} style={{ flex: 1, background: 'var(--bg-section)', border: '1px solid var(--border)', color: 'var(--gold)', cursor: 'pointer', fontSize: 12, padding: '7px', borderRadius: 6, fontFamily: 'inherit', fontWeight: 600 }}>✏️ Edit</button>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
        {Object.entries(byCategory).map(([cat, amt]) => (
          <span key={cat} style={{ background: 'var(--bg-section)', border: '1px solid var(--border)', borderRadius: 20, padding: '4px 12px', fontSize: 12, color: 'var(--text3)' }}>
      ...
      {isMobile ? (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          {expenses.length === 0 && (
            <div style={{ padding: 32, textAlign: 'center', color: 'var(--text3)', background: 'var(--bg)', border: '1px dashed var(--border)', borderRadius: 10 }}>
              <div style={{ fontSize: 28, marginBottom: 8, opacity: 0.5 }}>💳</div>
              <div style={{ fontSize: 13, color: 'var(--text2)' }}>No personal expenses recorded</div>
            </div>
          )}
          {expenses.map(e => (
            <div key={e.id} style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderLeft: '3px solid #ef4444', borderRadius: 10, padding: '12px 14px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 8, marginBottom: 6 }}>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontSize: 13, color: 'var(--text)', fontWeight: 600, marginBottom: 3, wordBreak: 'break-word' }}>{e.title}</div>
                  <div style={{ display: 'flex', gap: 6, alignItems: 'center', flexWrap: 'wrap' }}>
                    <span style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 10, padding: '1px 8px', fontSize: 10, color: 'var(--text2)' }}>{e.category}</span>
                    <span style={{ fontSize: 11, color: 'var(--text3)' }}>{fmtDate(e.expense_date)}</span>
                  </div>
                </div>
                <div style={{ fontSize: 16, fontWeight: 800, color: '#ef4444', letterSpacing: -0.3, whiteSpace: 'nowrap' }}>{fmt(e.amount)}</div>
              </div>
              {e.note && <div style={{ fontSize: 12, color: 'var(--text3)', marginTop: 4, fontStyle: 'italic' }}>{e.note}</div>}
              {canPersonalEdit && (
        ...
        <div style={S.section}>
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            ...
            <tbody>
              {expenses.length === 0 && <tr><td colSpan={6} style={{ padding: 40, textAlign: 'center', color: 'var(--text3)' }}>No personal expenses recorded</td></tr>}
              {expenses.map(e => (
                <tr key={e.id} style={{ borderBottom: '1px solid #1a1a1a' }}>
                  <td style={S.td}>{fmtDate(e.expense_date)}</td>
                  <td style={{ ...S.td, color: 'var(--text2)', fontWeight: 500 }}>{e.title}</td>
                  <td style={S.td}>
                    <span style={{ background: 'var(--bg-section)', border: '1px solid var(--border)', borderRadius: 4, padding: '2px 8px', fontSize: 11, color: 'var(--text3)' }}>{e.category}</span>
                  </td>
                  <td style={{ ...S.td, color: '#ef4444', fontWeight: 700 }}>{fmt(e.amount)}</td>
                  <td style={{ ...S.td, color: 'var(--text3)' }}>{e.note || '—'}</td>
                  <td style={S.td}>
                    {canPersonalEdit && <button onClick={() => del(e.id)} style={{ background: 'none', border: 'none', color: 'var(--text3)', cursor: 'pointer', fontSize: 16 }}>🗑</button>}
                  </td>
                </tr>
              ))}
            </tbody>
      ...
      {showModal && (
        <Modal title="💳 Personal Expense" onClose={() => setShowModal(false)}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            ...
            <button onClick={add} disabled={saving} style={{ ...S.btn, background: 'var(--gold)', color: 'var(--text)', fontWeight: 700, padding: '11px' }}>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
        ...
        <>
          {record ? (
            <div style={{ background: 'rgba(201,169,110,0.05)', border: '1px solid #c9a96e33', borderRadius: 14, padding: 24 }}>
              <div style={{ fontSize: 12, color: 'var(--gold)', fontFamily: 'monospace', letterSpacing: 1, textTransform: 'uppercase', marginBottom: 16 }}>
              ...
              <div style={{ background: 'var(--bg-card)', borderRadius: 100, height: 10, overflow: 'hidden', marginBottom: 6 }}>
              ...
              <div style={{ marginTop: 16, paddingTop: 16, borderTop: '1px solid var(--border)', display: 'flex', gap: 16, flexWrap: 'wrap' }}>
                {[
                  ...
                  <div key={l}>
                    ...
                    <span style={{ fontSize: 13, color: 'var(--text2)' }}>{fmt(v)}</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div style={{ ...S.card, textAlign: 'center', padding: 40, color: 'var(--text3)' }}>
              ...
              <div style={{ fontSize: 12, color: 'var(--text3)' }}>Click &ldquo;Calculate Zakat&rdquo; above to begin</div>
            </div>
          )}
          ...
          {record && (
            <div style={S.section}>
              <div style={{ padding: '14px 20px', borderBottom: '1px solid var(--border)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <span style={{ fontSize: 13, fontWeight: 600, color: 'var(--text2)' }}>🤲 Distribution History</span>
                <span style={{ fontSize: 12, color: 'var(--text3)' }}>{data?.distributions?.length || 0} entries</span>
              </div>
              <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                <thead><tr>{['Date', 'Recipient', 'Amount', 'Note', ''].map(h => <th key={h} style={S.th}>{h}</th>)}</tr></thead>
                <tbody>
                  {!data?.distributions?.length && <tr><td colSpan={5} style={{ padding: 40, textAlign: 'center', color: 'var(--text3)' }}>No distributions yet</td></tr>}
                  {data?.distributions?.map(d => (
                    <tr key={d.id} style={{ borderBottom: '1px solid #1a1a1a' }}>
                      <td style={S.td}>{fmtDate(d.distribution_date)}</td>
                      <td style={{ ...S.td, color: 'var(--text2)', fontWeight: 500 }}>{d.recipient}</td>
                      <td style={{ ...S.td, color: 'var(--gold)', fontWeight: 700 }}>{fmt(d.amount)}</td>
                      <td style={{ ...S.td, color: 'var(--text3)' }}>{d.note || '—'}</td>
                      <td style={S.td}>
                        {canZakatEdit && <button onClick={() => delDist(d.id)} style={{ background: 'none', border: 'none', color: 'var(--text3)', cursor: 'pointer', fontSize: 16 }}>🗑</button>}
                      </td>
                    </tr>
                  ))}
                </tbody>
      ...
      {showCalc && (
        <Modal title="🧮 Zakat Calculator" onClose={() => setShowCalc(false)} width={560}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            <div style={{ background: 'var(--bg-card)', border: '1px solid #c9a96e33', borderRadius: 8, padding: 14, fontSize: 12, color: 'var(--text2)' }}>
            ...
            <div style={{ fontSize: 12, color: 'var(--text3)', fontFamily: 'monospace', textTransform: 'uppercase', borderTop: '1px solid var(--border)', paddingTop: 14 }}>Assets</div>
            <div>
            ...
            <div style={{ background: 'var(--green-dim)', border: '1px solid #22c55e33', borderRadius: 10, padding: 16 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8 }}>
                <span style={{ fontSize: 13, color: 'var(--text3)' }}>Total Zakatable Assets:</span>
                <span style={{ fontSize: 14, fontWeight: 700, color: 'var(--gold)' }}>{fmt(calcTotal())}</span>
              </div>
              <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                <span style={{ fontSize: 13, color: 'var(--text3)' }}>Zakat Due (2.5%):</span>
            ...
            <button onClick={saveCalc} disabled={saving} style={{ ...S.btn, background: 'var(--gold)', color: 'var(--text)', fontWeight: 700, padding: '11px' }}>
      ...
      {showDist && (
        <Modal title="🤲 Zakat Distribution" onClose={() => setShowDist(false)}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            ...
            <div style={{ background: 'var(--bg-card)', borderRadius: 8, padding: 12, fontSize: 12, color: 'var(--text3)' }}>
              ⚠️ Distribution cannot exceed the remaining amount
            </div>
            <button onClick={addDist} disabled={saving} style={{ ...S.btn, background: 'var(--gold)', color: 'var(--text)', fontWeight: 700, padding: '11px' }}>
...
export default function AccountsPage() {
  ...
  if (TABS.length === 0) {
    return (
      <div style={{ padding: 60, textAlign: 'center', color: 'var(--text2)', fontFamily: "'Söhne', 'Helvetica Neue', sans-serif" }}>
        <div style={{ fontSize: 36, marginBottom: 12 }}>🔒</div>
        <div style={{ fontSize: 16, color: 'var(--text)', fontWeight: 600, marginBottom: 8 }}>Permission denied</div>
        <div style={{ fontSize: 13, color: 'var(--text3)' }}>You don&apos;t have access to any Accounts tabs. Please contact the CEO to grant access.</div>
      </div>
    );
  }
  ...
  return (
    <div style={{ padding: '24px 32px', maxWidth: 1300, fontFamily: "'Söhne', 'Helvetica Neue', sans-serif" }}>
      <div style={{ marginBottom: 24 }}>
        ...
        <div style={{ fontSize: 13, color: 'var(--text3)' }}>Financial dashboard, settlements, vendors &amp; zakat</div>
      </div>
