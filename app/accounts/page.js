                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8 }}>
                  <div>
                    ...
                    <div style={{ fontSize: 13, color: '#aaa', fontWeight: 600 }}>
                      {d.total ? 'Rs. ' + Math.round(d.total / 1000) + 'K' : '—'}
                    </div>
                  </div>
                  <div>
                    <div style={{ fontSize: 11, color: '#555', marginBottom: 2 }}>Settled</div>
                    <div style={{ fontSize: 13, color: '#4caf79', fontWeight: 600 }}>
                      {d.settled ? 'Rs. ' + Math.round(d.settled / 1000) + 'K' : '—'}
                    </div>
                  </div>
                  <div>
                    <div style={{ fontSize: 11, color: '#555', marginBottom: 2 }}>Pending</div>
                    <div style={{ fontSize: 13, color: '#e87d44', fontWeight: 600 }}>
                      {d.pending ? 'Rs. ' + Math.round(d.pending / 1000) + 'K' : '—'}
                    </div>
                  </div>
                </div>
...
function SettlementsTab() {
  ...
  const [form, setForm] = useState({ courier_name: 'PostEx', settlement_ref: '', amount: '', orders_count: '', settled_at: new Date().toISOString().split('T')[0], note: '' });
  ...
  const save = async () => {
    ...
    setSaving(true);
    await fetch('/api/accounts/settlements', {
      method: 'POST',
    ...
    setSaving(false);
    setShowForm(false);
    setForm({ courier_name: 'PostEx', settlement_ref: '', amount: '', orders_count: '', settled_at: new Date().toISOString().split('T')[0], note: '' });
    load();
  };
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', alignItems: 'center' }}>
        ...
        <button onClick={() => setShowForm(!showForm)} style={{ ...btnStyle, marginLeft: 'auto' }}>
          + Add Settlement
        </button>
      </div>
      ...
      {showForm && (
        <div style={{ background: '#111', border: '1px solid #c9a96e33', borderRadius: 12, padding: 20 }}>
          <div style={{ fontSize: 13, color: '#c9a96e', marginBottom: 16, fontWeight: 600 }}>New Settlement Entry</div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(180px, 1fr))', gap: 12 }}>
            <div>
              <label style={labelStyle}>Courier</label>
              <select value={form.courier_name} onChange={e => setForm({ ...form, courier_name: e.target.value })} style={inputStyle}>
                {COURIERS.map(c => <option key={c}>{c}</option>)}
              </select>
            </div>
            <div>
            ...
            <div>
              ...
              <input type="number" placeholder="0" value={form.orders_count} onChange={e => setForm({ ...form, orders_count: e.target.value })} style={inputStyle} />
            </div>
            <div>
              <label style={labelStyle}>Settlement Ref#</label>
              <input type="text" placeholder="e.g. PX-2024-001" value={form.settlement_ref} onChange={e => setForm({ ...form, settlement_ref: e.target.value })} style={inputStyle} />
            </div>
            <div>
              <label style={labelStyle}>Date</label>
              <input type="date" value={form.settled_at} onChange={e => setForm({ ...form, settled_at: e.target.value })} style={inputStyle} />
            </div>
            <div>
              <label style={labelStyle}>Note</label>
              <input type="text" placeholder="Optional note" value={form.note} onChange={e => setForm({ ...form, note: e.target.value })} style={inputStyle} />
            </div>
          </div>
          <div style={{ display: 'flex', gap: 10, marginTop: 16 }}>
            <button onClick={save} disabled={saving} style={{ ...btnStyle, background: '#c9a96e', color: '#000' }}>
              {saving ? 'Saving...' : 'Save Settlement'}
            </button>
            <button onClick={() => setShowForm(false)} style={{ ...btnStyle, background: '#1e1e1e', color: '#888' }}>
              Cancel
            </button>
          </div>
        </div>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          ...
          <tbody>
            {loading ? (
              ...
              <tr><td colSpan={7} style={{ textAlign: 'center', padding: 40, color: '#444' }}>No settlements recorded yet</td></tr>
            ) : settlements.map(s => (
              <tr key={s.id} style={{ borderBottom: '1px solid #1a1a1a' }}>
                ...
                <td style={{ ...tdStyle, color: '#555', fontSize: 12 }}>{s.note || '—'}</td>
                <td style={tdStyle}>
        ...
        <div style={{ background: '#111', border: '1px solid #c9a96e33', borderRadius: 12, padding: 20 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(180px, 1fr))', gap: 12 }}>
            <div>
              ...
              <input placeholder="Packaging bags" value={form.title} onChange={e => setForm({ ...form, title: e.target.value })} style={inputStyle} />
            </div>
            <div>
              <label style={labelStyle}>Amount (Rs.)</label>
              <input type="number" placeholder="0" value={form.amount} onChange={e => setForm({ ...form, amount: e.target.value })} style={inputStyle} />
            ...
            <div>
              ...
              <input placeholder="Abdul / Sharjeel..." value={form.paid_by} onChange={e => setForm({ ...form, paid_by: e.target.value })} style={inputStyle} />
            </div>
            <div>
              <label style={labelStyle}>Note</label>
              <input placeholder="Optional" value={form.note} onChange={e => setForm({ ...form, note: e.target.value })} style={inputStyle} />
            </div>
          </div>
          <div style={{ display: 'flex', gap: 10, marginTop: 16 }}>
            <button onClick={save} disabled={saving} style={{ ...btnStyle, background: '#c9a96e', color: '#000' }}>
          ...
          <tbody>
            {loading ? (
              ...
              <tr key={e.id} style={{ borderBottom: '1px solid #1a1a1a' }}>
                ...
                <td style={tdStyle}>
                  <span style={{ background: '#1e1e1e', border: '1px solid #2a2a2a', borderRadius: 4, padding: '2px 8px', fontSize: 11, color: '#888' }}>{e.category}</span>
                </td>
                ...
                <td style={tdStyle}>
                  <button onClick={() => del(e.id)} style={{ background: 'none', border: 'none', color: '#444', cursor: 'pointer', fontSize: 16 }}>🗑</button>
                </td>
              </tr>
            ))}
          </tbody>
    ...
    <div style={{ display: 'flex', gap: 20 }}>
      ...
      <div style={{ width: 300, flexShrink: 0 }}>
        <div style={{ display: 'flex', alignItems: 'center', marginBottom: 12 }}>
          <span style={{ fontSize: 13, color: '#666' }}>
            Total Outstanding: <span style={{ color: '#9b7fe8', fontWeight: 700 }}>{fmt(totalOutstanding)}</span>
          </span>
          <button onClick={() => { setSelected(null); setShowForm(true); }} style={{ ...btnStyle, marginLeft: 'auto', fontSize: 11, padding: '4px 10px' }}>
            + New Vendor
          </button>
        </div>
        ...
        {loading ? (
          ...
          <div style={{ color: '#444', textAlign: 'center', padding: 40 }}>No vendors yet</div>
        ) : ledger.map(v => (
          <div key={v.id} onClick={() => loadTxns(v)}
            style={{
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
              ...
              <span style={{ fontSize: 12, color: v.outstanding > 0 ? '#9b7fe8' : '#4caf79', fontWeight: 600 }}>
      ...
      <div style={{ flex: 1 }}>
        {!selected && !showForm ? (
          ...
          <div style={{ background: '#111', border: '1px solid #c9a96e33', borderRadius: 12, padding: 24 }}>
            <div style={{ fontSize: 14, color: '#c9a96e', marginBottom: 20, fontWeight: 600 }}>
              {selected ? `Add Transaction — ${selected.name}` : 'New Vendor'}
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
              {!selected && (
                <div style={{ gridColumn: '1/-1' }}>
                  <label style={labelStyle}>Vendor Name</label>
                  <input placeholder="e.g. Ali Zeenat Traders" value={form.vendor_name} onChange={e => setForm({ ...form, vendor_name: e.target.value })} style={inputStyle} />
                </div>
              )}
              <div>
              ...
              <div>
                ...
                <input placeholder="Invoice/Challan #" value={form.reference} onChange={e => setForm({ ...form, reference: e.target.value })} style={inputStyle} />
              </div>
              <div style={{ gridColumn: '1/-1' }}>
                <label style={labelStyle}>Note</label>
                <input placeholder="Optional" value={form.note} onChange={e => setForm({ ...form, note: e.target.value })} style={inputStyle} />
            ...
            <div style={{ display: 'flex', alignItems: 'center', marginBottom: 16 }}>
              <div>
                ...
                <div style={{ fontSize: 12, color: '#555', marginTop: 2 }}>
                  Outstanding: <span style={{ color: '#9b7fe8' }}>{fmt(selected.outstanding)}</span>
                </div>
              </div>
                ...
                <tbody>
                  {txns.length === 0 ? (
                    ...
                    <tr key={t.id} style={{ borderBottom: '1px solid #1a1a1a' }}>
                      ...
                      <td style={tdStyle}>
                        <span style={{
                          background: t.payment_type === 'purchase' ? '#3a1e1e' : '#1e3a2e',
                          color: t.payment_type === 'purchase' ? '#e84444' : '#4caf79',
                          padding: '2px 8px', borderRadius: 4, fontSize: 11, textTransform: 'uppercase',
                        }}>
                          {t.payment_type === 'purchase' ? 'Debit' : 'Credit'}
                        </span>
                      </td>
                      <td style={{ ...tdStyle, color: t.payment_type === 'purchase' ? '#e84444' : '#4caf79', fontWeight: 600 }}>
                        {t.payment_type === 'purchase' ? '-' : '+'}{fmt(t.amount)}
                      </td>
  ...
  return (
    <div style={{ padding: '24px 32px', maxWidth: 1200, fontFamily: "'Söhne', 'Helvetica Neue', sans-serif" }}>
      ...
      <div style={{ marginBottom: 28 }}>
        <div style={{ fontSize: 22, fontWeight: 700, color: '#eee', letterSpacing: -0.5, marginBottom: 4 }}>
          Accounts
        </div>
