...
const btnStyle = {
  background: 'var(--bg-section)', border: '1px solid var(--border)', borderRadius: 8,
  padding: '8px 16px', color: 'var(--gold)', fontSize: 13, cursor: 'pointer',
...
const tdStyle = { padding: '12px 16px', fontSize: 13, color: 'var(--text2)', verticalAlign: 'middle' };

// ─── MODAL WRAPPER ─────────────────────────────────────────────
function Modal({ title, onClose, children, maxWidth = 480 }) {
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.75)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
      <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 14, padding: 28, width: '100%', maxWidth, maxHeight: '90vh', overflowY: 'auto' }}>
...
function BillPreview({ url, size = 36 }) {
  if (!url) return <span style={{ color: 'var(--text3)', fontSize: 12 }}>—</span>;
  if (isImageUrl(url)) {
    return (
  ...
  return (
    <a href={url} target="_blank" rel="noopener noreferrer"
      style={{ color: 'var(--gold)', fontSize: 18, textDecoration: 'none' }} title="Bill dekho (PDF)">
      📎
    </a>
  );
}
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
      ...
      <div>
        ...
        <div style={{ border: '1px dashed var(--border)', borderRadius: 8, padding: 14, textAlign: 'center', background: 'var(--bg-card)', cursor: 'pointer', position: 'relative' }}
          ...
          {newPreviewUrl ? (
            ...
            <div>
              ...
              <div style={{ fontSize: 11, color: 'var(--text3)' }}>📎 Existing bill — naya upload karne ke liye click karo</div>
            </div>
          ) : existingUrl ? (
            <div>
              <div style={{ fontSize: 22, marginBottom: 4 }}>📎</div>
              <div style={{ fontSize: 12, color: 'var(--text3)' }}>Existing bill attached — click to replace</div>
            </div>
          ) : (
            <div>
              <div style={{ fontSize: 22, marginBottom: 4 }}>📎</div>
              <div style={{ fontSize: 12, color: 'var(--text3)' }}>Click karke bill upload karo</div>
              <div style={{ fontSize: 10, color: 'var(--text3)', marginTop: 4 }}>Image ya PDF — optional</div>
            </div>
          )}
        </div>
        {(form.bill_file || form.bill_url) && (
          <button onClick={() => setForm(f => ({ ...f, bill_file: null, bill_preview: null, bill_url: null, _clear_bill: true }))}
            style={{ marginTop: 6, background: 'none', border: 'none', color: 'var(--text3)', fontSize: 11, cursor: 'pointer' }}>
            ✕ Remove bill
          </button>
        )}
      </div>
      <div style={{ display: 'flex', gap: 10 }}>
        ...
        <button onClick={onCancel} style={{ ...btnStyle, color: 'var(--text3)' }}>Cancel</button>
      </div>
    </div>
  );
}
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: 12 }}>
        <div style={{ background: 'var(--bg-card)', border: `1px solid ${balanceColor}33`, borderRadius: 12, padding: '20px 24px' }}>
          <div style={{ fontSize: 11, color: 'var(--text3)', fontFamily: 'monospace', letterSpacing: 1, textTransform: 'uppercase', marginBottom: 8 }}>Cash Balance</div>
          <div style={{ fontSize: 28, fontWeight: 700, color: balanceColor, letterSpacing: -0.5 }}>{fmt(data.balance)}</div>
        </div>
        <div style={{ background: 'var(--bg-card)', border: '1px solid #1a2a1a', borderRadius: 12, padding: '20px 24px' }}>
          <div style={{ fontSize: 11, color: 'var(--text3)', fontFamily: 'monospace', letterSpacing: 1, textTransform: 'uppercase', marginBottom: 8 }}>Total Cash In</div>
          <div style={{ fontSize: 22, fontWeight: 700, color: '#22c55e' }}>{fmt(data.total_in)}</div>
        </div>
        <div style={{ background: 'var(--bg-card)', border: '1px solid #2a1a1a', borderRadius: 12, padding: '20px 24px' }}>
          <div style={{ fontSize: 11, color: 'var(--text3)', fontFamily: 'monospace', letterSpacing: 1, textTransform: 'uppercase', marginBottom: 8 }}>Total Out</div>
          <div style={{ fontSize: 22, fontWeight: 700, color: '#ef4444' }}>{fmt(data.total_out)}</div>
        </div>
        <div style={{ background: 'var(--bg-card)', border: '1px solid #ef444433', borderRadius: 12, padding: '20px 24px' }}>
          <div style={{ fontSize: 11, color: 'var(--text3)', fontFamily: 'monospace', letterSpacing: 1, textTransform: 'uppercase', marginBottom: 8 }}>🗓 Aaj Ka Kharch</div>
          <div style={{ fontSize: 22, fontWeight: 700, color: todayExpense > 0 ? '#ef4444' : '#333' }}>{fmt(todayExpense)}</div>
          <div style={{ fontSize: 10, color: 'var(--text3)', marginTop: 4 }}>{todayLogs.length} transactions</div>
        </div>
        <div style={{ background: 'var(--bg-card)', border: '1px solid #f59e0b33', borderRadius: 12, padding: '20px 24px' }}>
          <div style={{ fontSize: 11, color: 'var(--text3)', fontFamily: 'monospace', letterSpacing: 1, textTransform: 'uppercase', marginBottom: 8 }}>📅 Is Mahine Ka Kharch</div>
          <div style={{ fontSize: 22, fontWeight: 700, color: '#f59e0b' }}>{fmt(monthExpense)}</div>
          <div style={{ fontSize: 10, color: 'var(--text3)', marginTop: 4 }}>{monthLogs.length} transactions</div>
        </div>
        {data.pending_count > 0 && (
          <div style={{ background: 'var(--bg-card)', border: '1px solid #c9a96e33', borderRadius: 12, padding: '20px 24px' }}>
            <div style={{ fontSize: 11, color: 'var(--text3)', fontFamily: 'monospace', letterSpacing: 1, textTransform: 'uppercase', marginBottom: 8 }}>Pending Approvals</div>
            <div style={{ fontSize: 22, fontWeight: 700, color: 'var(--gold)' }}>{data.pending_count}</div>
          </div>
        )}
      </div>
      ...
      <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', alignItems: 'center' }}>
        ...
        <div style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          {month && <button onClick={() => setMonth('')} style={{ ...btnStyle, padding: '6px 10px', fontSize: 11, color: 'var(--text3)' }}>Clear</button>}
        </div>
      </div>
      ...
      {isCEO && pendingLogs.length > 0 && (
        <div style={{ background: 'var(--bg-card)', border: '1px solid #c9a96e44', borderRadius: 12, overflow: 'hidden' }}>
          <div style={{ padding: '14px 20px', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', gap: 8 }}>
            <span style={{ fontSize: 13, fontWeight: 600, color: 'var(--gold)' }}>⏳ Pending Cash Requests</span>
            <span style={{ background: '#c9a96e22', color: 'var(--gold)', borderRadius: 20, padding: '1px 8px', fontSize: 11 }}>{pendingLogs.length}</span>
          </div>
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              <tr style={{ borderBottom: '1px solid var(--border)' }}>
                {['Date', 'Requested By', 'Amount', 'Notes', 'Action'].map(h => (
                  <th key={h} style={{ padding: '10px 16px', textAlign: 'left', fontSize: 11, color: 'var(--text3)', fontFamily: 'monospace', letterSpacing: 1, textTransform: 'uppercase', fontWeight: 400 }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {pendingLogs.map(l => (
                <tr key={l.id} style={{ borderBottom: '1px solid #1a1a1a' }}>
                  ...
                  <td style={{ ...tdStyle, color: 'var(--text2)' }}>{l.requested_by}</td>
                  <td style={{ ...tdStyle, color: '#22c55e', fontWeight: 700 }}>{fmt(l.amount)}</td>
                  <td style={{ ...tdStyle, color: 'var(--text3)' }}>{l.notes || '—'}</td>
                  <td style={{ ...tdStyle }}>
      ...
      <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 12, overflow: 'hidden' }}>
        <div style={{ padding: '14px 20px', borderBottom: '1px solid var(--border)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div>
            <span style={{ fontSize: 13, fontWeight: 600, color: 'var(--text2)' }}>Transaction History</span>
            <span style={{ marginLeft: 10, fontSize: 12, color: 'var(--text3)' }}>{approvedLogs.length} entries</span>
          </div>
        </div>
        {loading ? (
          <div style={{ padding: 40, textAlign: 'center', color: 'var(--text3)' }}>Loading...</div>
        ) : approvedLogs.length === 0 ? (
          <div style={{ padding: 40, textAlign: 'center', color: 'var(--text3)' }}>Koi transaction nahi</div>
        ) : (
          <div>
            {sortedDates.map(date => {
              ...
              return (
                <div key={date}>
                  <div style={{ padding: '10px 20px', background: 'var(--bg)', borderBottom: '1px solid #1a1a1a', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                      <span style={{ fontSize: 12, fontWeight: 700, color: isToday ? '#c9a96e' : '#555' }}>
                        {isToday ? '📅 Aaj — ' : ''}{fmtDate(date)}
                      </span>
                      <span style={{ fontSize: 11, color: 'var(--text3)' }}>{dayLogs.length} transactions</span>
                    </div>
                          ...
                          <tr key={l.id} style={{ borderBottom: '1px solid #1a1a1a' }}>
                            ...
                            <td style={{ ...tdStyle, color: 'var(--text2)' }}>
                              ...
                              <div style={{ display: 'flex', gap: 8, alignItems: 'center', marginTop: 3 }}>
                                {l.requested_by && (
                                  <span style={{ fontSize: 10, color: 'var(--text3)' }}>by {l.requested_by}</span>
                                )}
                            ...
                            <td style={{ ...tdStyle }}>
                              {l.category ? <span style={{ background: 'var(--bg-section)', border: '1px solid var(--border)', borderRadius: 4, padding: '2px 8px', fontSize: 11, color: 'var(--text3)' }}>{l.category}</span> : '—'}
                            </td>
        ...
        <Modal title="💰 Cash Request" onClose={() => setShowCashModal(false)}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              ...
              <button onClick={() => setShowCashModal(false)} style={{ ...btnStyle, color: 'var(--text3)' }}>Cancel</button>
            </div>
          </div>
        </Modal>
      )}
      ...
      {editingEntry && (
        <Modal title={`✏️ Entry Edit karo — ${editingEntry.description}`} onClose={() => setEditingEntry(null)}>
          <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 8, padding: 10, fontSize: 11, color: 'var(--text3)', marginBottom: 14 }}>
      ...
      {showHistoryFor && (
        <Modal title={`📜 Edit History — ${showHistoryFor.description}`} onClose={() => setShowHistoryFor(null)} maxWidth={560}>
          <div style={{ fontSize: 12, color: 'var(--text3)', marginBottom: 16 }}>
            Original by <span style={{ color: 'var(--text2)' }}>{showHistoryFor.requested_by || 'Unknown'}</span>
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            {(showHistoryFor.edit_history || []).slice().reverse().map((h, idx) => (
              <div key={idx} style={{ background: 'var(--bg)', border: '1px solid var(--border)', borderRadius: 8, padding: 14 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
                  <span style={{ fontSize: 12, color: '#8b5cf6', fontWeight: 600 }}>✏️ {h.edited_by}</span>
                  <span style={{ fontSize: 11, color: 'var(--text3)' }}>{fmtDateTime(h.edited_at)}</span>
                </div>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                  {Object.entries(h.changes || {}).map(([field, diff]) => (
                    <div key={field} style={{ fontSize: 12, background: 'var(--bg-card)', borderRadius: 6, padding: '6px 10px' }}>
                      <div style={{ color: 'var(--text2)', fontWeight: 600, marginBottom: 2, fontFamily: 'monospace', fontSize: 11 }}>{field}</div>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      <div style={{ display: 'flex', gap: 12, alignItems: 'center', flexWrap: 'wrap' }}>
        <div style={{ background: 'var(--bg-card)', border: '1px solid #2a1a1a', borderRadius: 12, padding: '18px 24px' }}>
          <div style={{ fontSize: 11, color: 'var(--text3)', fontFamily: 'monospace', letterSpacing: 1, textTransform: 'uppercase', marginBottom: 6 }}>Total Outstanding</div>
      ...
      <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 12, overflow: 'hidden' }}>
        <div style={{ padding: '14px 20px', borderBottom: '1px solid var(--border)' }}>
          <span style={{ fontSize: 13, fontWeight: 600, color: 'var(--text2)' }}>Advances History</span>
          <span style={{ marginLeft: 10, fontSize: 12, color: 'var(--text3)' }}>{advances.length} records</span>
        </div>
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          <thead>
            <tr style={{ borderBottom: '1px solid var(--border)' }}>
          ...
          <tbody>
            {advances.length === 0 ? (
              <tr><td colSpan={7} style={{ padding: 40, textAlign: 'center', color: 'var(--text3)' }}>Koi advance nahi</td></tr>
            ) : advances.map(a => (
              <tr key={a.id} style={{ borderBottom: '1px solid #1a1a1a' }}>
                <td style={{ ...tdStyle, color: 'var(--text2)', fontWeight: 500 }}>{a.employees?.name || '—'}</td>
                ...
                <td style={{ ...tdStyle, color: 'var(--text3)' }}>{a.notes || '—'}</td>
                <td style={tdStyle}>
                  {a.status === 'pending' && (
        ...
        <Modal title="💸 Advance Do" onClose={() => setShowModal(false)}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              ...
              <button onClick={() => setShowModal(false)} style={{ ...btnStyle, color: 'var(--text3)' }}>Cancel</button>
            </div>
          </div>
        </Modal>
      )}
  ...
  return (
    <div style={{ padding: '24px 32px', maxWidth: 1200, fontFamily: "'Söhne', 'Helvetica Neue', sans-serif" }}>
      <div style={{ marginBottom: 28 }}>
        ...
        <div style={{ fontSize: 13, color: 'var(--text3)' }}>Cash wallet, daily expenses & staff advances</div>
      </div>
