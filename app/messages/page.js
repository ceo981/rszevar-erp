  ...
  return (
    ...
    >
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        {subtitle && (
          <div style={{ fontSize: 10, color: 'var(--text3)', marginTop: 1, marginBottom: 3 }}>
            {subtitle}
          </div>
        )}
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 8, marginTop: 3 }}>
          <span style={{
            ...
            {conv.last_message_direction === 'out' && <span style={{ color: 'var(--text3)' }}>↗ </span>}
  ...
  return (
    <div style={{ maxWidth: 280 }}>
      <video
        controls
        src={url}
        style={{ width: '100%', maxWidth: 280, borderRadius: 6, background: 'var(--bg)' }}
      >
...
function InboundDocument({ url, filename, size, mimeType }) {
  ...
  return (
    ...
    >
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ fontSize: 12, color: 'var(--text)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{name}</div>
...
function OrderChip({ order }) {
  ...
  return (
    ...
    >
      ...
      <span style={{ color: 'var(--text2)' }}>Rs {Number(order.total_amount || 0).toLocaleString()}</span>
    </a>
  );
}
...
function VoiceRecorder({ onSend, onCancel }) {
  ...
  return (
    <div style={{
      ...
      <span style={{ color: 'var(--text2)', fontSize: 13, fontFamily: 'monospace' }}>{formatDuration(duration)}</span>
      ...
      <button
        ...
        title="Cancel"
        style={{
          background: 'var(--red-dim)', border: '1px solid #4a2d2d', color: '#f87171',
...
export default function MessagesPage() {
  ...
  return (
    <div style={{ padding: 20, fontFamily: 'inherit', color: 'var(--text)', minHeight: 'calc(100vh - 60px)' }}>
      {/* Header */}
      ...
      <div style={{
          ...
          <div style={{ padding: 12, borderBottom: `1px solid ${border}` }}>
            <input
              ...
              style={{
                ...
                color: 'var(--text)', borderRadius: 6, padding: '8px 12px', fontSize: 12,
          ...
          {(() => {
            ...
            return (
              <div style={{
                ...
                borderBottom: `1px solid ${border}`, background: 'var(--bg-card)',
              }}>
                {tabs.map(t => {
        ...
        <div style={{
          ...
          {!selectedId ? (
            ...
            <>
              {/* Chat header */}
              <div style={{ padding: '12px 16px', borderBottom: `1px solid ${border}`, background: 'var(--bg-card)', position: 'relative' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontSize: 14, fontWeight: 600, color: 'var(--text)' }}>{selectedName}</div>
              ...
              <div style={{
                ...
                {loadingMsgs && messages.length === 0 && (
                  <div style={{ textAlign: 'center', color: 'var(--text3)', fontSize: 12, padding: 30 }}>Loading messages...</div>
                )}
                {!loadingMsgs && messages.length === 0 && (
                  <div style={{ textAlign: 'center', color: 'var(--text3)', fontSize: 12, padding: 30 }}>No messages in this conversation yet</div>
                )}
                {messages.map(msg => (
                  <MessageBubble
                    key={msg.id}
              ...
              <div style={{ padding: 12, borderTop: `1px solid ${border}`, background: 'var(--bg-card)' }}>
                {!canReply && (
                  <div style={{ fontSize: 12, color: 'var(--text2)', textAlign: 'center', padding: '14px 8px', background: 'var(--bg-card)', border: `1px solid ${border}`, borderRadius: 8 }}>
                    🔒 Reply karne ki ijazat tumhe nahi hai. CEO se <code style={{ background: 'var(--bg)', padding: '1px 5px', borderRadius: 3, fontSize: 11 }}>messages.reply</code> permission grant karwane ko bolo.
                  </div>
                )}
                {canReply && (<>
                {sendError && (
                ...
                {recording ? (
                  ...
                  <div style={{ display: 'flex', gap: 6, alignItems: 'flex-end' }}>
                    ...
                    <button
                      ...
                      className="emoji-font"
                      style={{
                        background: 'var(--bg-card)', border: `1px solid ${border}`, color: 'var(--text2)',
                    ...
                    <textarea
                      ...
                      rows={2}
                      style={{
                        flex: 1, background: 'var(--bg)', border: `1px solid ${border}`,
