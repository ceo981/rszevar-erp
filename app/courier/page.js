...
const STATUS_COLORS = {
  ...
  cancelled: { color: 'var(--text3)',    bg: '#55555522', label: 'Cancelled' },
...
function CustomerDrawer({ customer, onClose, onRefresh }) {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 1000, display: 'flex' }}>
      ...
      <div style={{ width: 460, background: 'var(--bg-card)', borderLeft: `1px solid ${border}`, overflowY: 'auto', display: 'flex', flexDirection: 'column' }}>
        {/* Header */}
        <div style={{ padding: '20px 24px', borderBottom: `1px solid ${border}`, display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
          <div>
            ...
            <div style={{ fontSize: 12, color: 'var(--text3)', marginTop: 3 }}>{customer.phone} · {customer.city}</div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 1, borderBottom: `1px solid ${border}` }}>
          {[
            ...
            { label: 'Last Order', value: timeAgo(customer.last_order), color: 'var(--text2)' },
          ].map(s => (
        ...
        <div style={{ padding: '20px 24px', flex: 1 }}>
                ...
                {!showBlacklistForm ? (
                  ...
                  <div style={{ background: 'var(--red-dim)', border: '1px solid #330000', borderRadius: 8, padding: '14px' }}>
                    ...
                    <div style={{ display: 'flex', gap: 6 }}>
                      ...
                      <button onClick={() => setShowBlacklistForm(false)} style={{ background: 'none', border: `1px solid ${border}`, color: 'var(--text3)', borderRadius: 6, padding: '7px 14px', fontSize: 12, cursor: 'pointer', fontFamily: 'inherit' }}>Cancel</button>
                    </div>
                  </div>
                )}
              </div>
          ...
          {/* Order history */}
          {loading ? (
            <div style={{ color: 'var(--text3)', textAlign: 'center', padding: 30 }}>Loading...</div>
          ) : (
            <>
              <div style={{ fontWeight: 600, fontSize: 13, marginBottom: 10, color: 'var(--text2)' }}>Order History ({detail?.orders?.length || 0})</div>
                  ...
                  return (
                    <div key={i} style={{ background: card, border: `1px solid ${border}`, borderRadius: 8, padding: '12px 14px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      <div>
                        ...
                        <div style={{ fontSize: 11, color: 'var(--text3)', marginTop: 2 }}>{timeAgo(o.created_at)}</div>
                      </div>
                ...
                <>
                  ...
                  {detail.complaints.map((c, i) => (
                    <div key={i} style={{ background: 'var(--red-dim)', border: '1px solid #330000', borderRadius: 8, padding: '10px 14px', marginBottom: 6, fontSize: 12 }}>
                      ...
                      <div style={{ color: 'var(--text2)', marginTop: 2 }}>{c.description}</div>
                    </div>
                  ))}
                </>
              )}
  ...
  return (
    <div style={{ fontFamily: 'Inter, sans-serif', color: 'var(--text)', padding: 24 }}>
      ...
      <div style={{ marginBottom: 20 }}>
        ...
        <p style={{ margin: '4px 0 0', fontSize: 13, color: 'var(--text3)' }}>{summary.total || 0} unique customers · from order history</p>
      </div>
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 14, flexWrap: 'wrap' }}>
        {filters.map(f => (
          <button key={f.id} onClick={() => { setFilter(f.id); setPage(1); }}
            style={{ padding: '7px 14px', background: filter === f.id ? '#1e1e1e' : 'transparent', border: `1px solid ${filter === f.id ? 'var(--border)' : border}`, borderRadius: 8, fontSize: 12, color: filter === f.id ? gold : '#555', cursor: 'pointer', fontFamily: 'inherit' }}>
      ...
      <div className="mobile-card-table" style={{ background: card, border: `1px solid ${border}`, borderRadius: 10, overflow: 'hidden' }}>
            ...
            <thead>
              <tr style={{ borderBottom: `1px solid ${border}` }}>
                {['Customer', 'Phone', 'City', 'Orders', 'Total Spend', 'Delivered', 'RTO', 'Last Order', 'Tag'].map(h => (
                  <th key={h} style={{ padding: '11px 14px', textAlign: 'left', color: 'var(--text3)', fontWeight: 500, fontSize: 11, textTransform: 'uppercase', letterSpacing: 0.5, background: 'var(--bg)' }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {loading && <tr><td colSpan={9} style={{ padding: 40, textAlign: 'center', color: 'var(--text3)' }}>Loading...</td></tr>}
              {!loading && customers.length === 0 && <tr><td colSpan={9} style={{ padding: 40, textAlign: 'center', color: 'var(--text3)' }}>No customers found</td></tr>}
              {customers.map((c, i) => (
                <tr key={i} onClick={() => setSelected(c)} style={{ borderBottom: '1px solid #1a1a1a', cursor: 'pointer', background: c.is_blacklisted ? '#1a000088' : 'transparent' }}>
                  ...
                  <td style={{ padding: '11px 14px', color: 'var(--text3)', fontSize: 12 }}>{c.phone}</td>
                  <td style={{ padding: '11px 14px', color: 'var(--text2)' }}>{c.city || '—'}</td>
                  ...
                  <td style={{ padding: '11px 14px', color: 'var(--text3)', fontSize: 12 }}>{timeAgo(c.last_order)}</td>
                  <td style={{ padding: '11px 14px' }}>
                    ...
                    {!c.is_vip && !c.is_repeat && !c.is_blacklisted && <span style={{ color: 'var(--text3)', fontSize: 11 }}>New</span>}
                  </td>
                </tr>
              ))}
            </tbody>
        ...
        <div className="mobile-card-view">
          ...
          {!loading && customers.length === 0 && <div style={{ padding: 40, textAlign: 'center', color: 'var(--text3)', fontSize: 13 }}>No customers found</div>}
          {customers.map((c, i) => (
            <div
              key={i}
            ...
            >
              <div className="mobile-card-row-header">
                ...
                <div style={{ textAlign: 'right', flexShrink: 0 }}>
                  ...
                  <div style={{ fontSize: 10, color: 'var(--text3)' }}>{timeAgo(c.last_order)}</div>
                </div>
              </div>
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, fontSize: 11 }}>
                <span style={{ padding: '2px 8px', borderRadius: 4, background: 'var(--bg-card)', color: 'var(--text)' }}>{c.orders} orders</span>
        ...
        <div style={{ padding: '12px 16px', borderTop: `1px solid ${border}`, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <span style={{ fontSize: 12, color: 'var(--text3)' }}>{total} customers</span>
          <div style={{ display: 'flex', gap: 6 }}>
            <button onClick={() => setPage(p => Math.max(1, p-1))} disabled={page === 1} style={{ background: 'var(--bg-card)', border: `1px solid ${border}`, color: page===1?'#333':'#888', borderRadius: 6, padding: '5px 12px', fontSize: 12, cursor: page===1?'not-allowed':'pointer' }}>← Prev</button>
            <span style={{ fontSize: 12, color: 'var(--text3)', padding: '5px 10px' }}>Page {page}</span>
            <button onClick={() => setPage(p => p+1)} disabled={customers.length < 30} style={{ background: 'var(--bg-card)', border: `1px solid ${border}`, color: customers.length<30?'#333':'#888', borderRadius: 6, padding: '5px 12px', fontSize: 12, cursor: customers.length<30?'not-allowed':'pointer' }}>Next →</button>
          </div>
        </div>
      </div>
    </div>
