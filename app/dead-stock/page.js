...
export default function DeadStockPage() {
  ...
  const exportCSV = () => {
    ...
    const headers = ['Title', 'SKU', 'Collection', 'ABC (90d)', 'Stock Qty', 'Price', 'Stock Value', 'Last Sale Date', 'Days Dead'];
    ...
    <div style={{ minHeight: '100vh', background: '#0a0a0a', color: '#e5e5e5', padding: '24px', fontFamily: 'Inter, sans-serif' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24 }}>
        ...
        <div style={{ display: 'flex', gap: 10 }}>
          <button onClick={fetchData} disabled={loading}
            style={{ padding: '8px 16px', background: '#1f2937', color: '#e5e5e5', border: '1px solid #374151', borderRadius: 8, cursor: 'pointer', fontSize: 13 }}>
            {loading ? '⏳ Loading...' : '🔄 Refresh'}
          </button>
          <button onClick={exportCSV}
            style={{ padding: '8px 16px', background: '#c9a96e', color: '#000', border: 'none', borderRadius: 8, cursor: 'pointer', fontSize: 13, fontWeight: 600 }}>
            ⬇ Export CSV
          </button>
        </div>
      </div>
            ...
            <thead>
              <tr style={{ background: '#1f2937', borderBottom: '1px solid #374151' }}>
                {['#', 'Product', 'Collection', 'ABC', 'Qty', 'Price', 'Stock Value', 'Last Sale', 'Days Dead'].map(h => (
                  <th key={h} style={{ padding: '12px 14px', textAlign: ['#','Qty','Price','Stock Value'].includes(h) ? 'center' : 'left', color: '#9ca3af', fontWeight: 600, fontSize: 12, whiteSpace: 'nowrap' }}>
                    {h}
                  </th>
                ))}
              </tr>
              ...
              {filtered.length === 0 ? (
                ...
                <tr key={p.id}
                  ...
                  <td style={{ padding: '10px 14px', maxWidth: 220 }}>
                    ...
                    {p.sku && p.sku !== '—' && <div style={{ fontSize: 11, color: '#6b7280', marginTop: 2 }}>SKU: {p.sku}</div>}
                  </td>
                  <td style={{ padding: '10px 14px', color: '#9ca3af', fontSize: 12 }}>{p.collection_name}</td>
                  <td style={{ padding: '10px 14px', textAlign: 'center' }}>{abcBadge(p.abc_90d)}</td>
                  <td style={{ padding: '10px 14px', textAlign: 'center', color: '#e5e5e5', fontWeight: 600 }}>{p.stock_quantity}</td>
                  <td style={{ padding: '10px 14px', textAlign: 'center', color: '#9ca3af' }}>{formatCurrency(p.selling_price)}</td>
      ...
      {!loading && filtered.length > 0 && (
        <div style={{ marginTop: 16, color: '#4b5563', fontSize: 12, textAlign: 'center' }}>
          📊 Based on 90-day ABC window · D-class = no sales in last 90 days
        </div>
      )}
    </div>
  );
