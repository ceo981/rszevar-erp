  ...
  if (!canView) {
    return (
      <div style={{ fontFamily:'Inter,sans-serif', background: 'var(--bg)', minHeight:'100vh', padding:60, color: 'var(--text)', maxWidth:480, margin:'0 auto', textAlign:'center' }}>
        ...
        <Link href="/" style={{ background: 'transparent', border: '1px solid var(--border)', color: 'var(--text2)', borderRadius: 6, padding: '8px 14px', fontSize: 12, fontWeight: 500, cursor: 'pointer', fontFamily: 'inherit', textDecoration: 'none', display: 'inline-block' }}>
          ← Wapas
        </Link>
      </div>
    );
  ...
  return (
    <div style={{ fontFamily:'Inter,sans-serif', background: 'var(--bg)', minHeight:'100vh', padding:'24px 20px', color: 'var(--text)', maxWidth:480, margin:'0 auto' }}>
      <div style={{ textAlign:'center', marginBottom:32 }}>
        ...
        <div style={{ fontSize:13, color: 'var(--text3)', marginTop:4 }}>Order number dalao, naam chuno, done</div>
      </div>
        ...
        <div style={{ marginBottom:24 }}>
          <div style={{ display:'flex', gap:10 }}>
            ...
            <button onClick={search} disabled={searching||!orderNum}
              style={{ background:searching?'#1a1a1a':gold, border: 'none', color: 'var(--text)', borderRadius:10, padding:'16px 22px', fontSize:20, fontWeight:700, cursor:searching?'not-allowed':'pointer' }}>
              {searching?'⟳':'🔍'}
            </button>
          </div>
      ...
      {order && !done && (
        <>
          <div style={{ background: 'var(--bg-card)', border: `1px solid ${gold}44`, borderRadius:12, padding:'16px 18px', marginBottom:14 }}>
            <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start' }}>
              <div>
                ...
                <div style={{ fontSize:14, color: 'var(--text2)', marginTop:3 }}>{order.customer_name} · {order.customer_city}</div>
                <div style={{ fontSize:13, color: 'var(--text3)', marginTop:2 }}>Rs {Number(order.total_amount).toLocaleString()}</div>
              </div>
              <button onClick={() => { setOrder(null); setItems([]); setOrderNum('ZEVAR-'); setPackedBy(''); setMsg(''); }}
                style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', color: 'var(--text3)', borderRadius:7, padding:'6px 12px', fontSize:12, cursor:'pointer' }}>✕</button>
            </div>
          </div>

          {items.length > 0 && (
            <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius:12, padding:14, marginBottom:16 }}>
              <div style={{ fontSize:11, color: 'var(--text3)', textTransform:'uppercase', letterSpacing:1, marginBottom:10 }}>Items ({items.length})</div>
              {items.map((item, i) => (
                <div key={i} style={{ display:'flex', alignItems:'center', gap:10, padding:'8px 0', borderBottom:i<items.length-1?'1px solid #1a1a1a':'none' }}>
                  ...
                  <div style={{ flex:1, minWidth:0 }}>
                    ...
                    {item.sku && <div style={{ fontSize:11, color: 'var(--text3)', marginTop:1 }}>SKU: {item.sku}</div>}
                  </div>
          ...
          <div style={{ fontSize:12, color: 'var(--text3)', marginBottom:10, textTransform:'uppercase', letterSpacing:1 }}>Packed By</div>
          <div style={{ display:'flex', flexDirection:'column', gap:8, marginBottom:20 }}>
            {team.map(emp => (
              <button key={emp.id} onClick={() => setPackedBy(String(emp.id))}
                style={{ background:packedBy===String(emp.id)?gold+'22':'#141414', border: `2px solid ${packedBy===String(emp.id)?gold:'var(--border)'}`, color:packedBy===String(emp.id)?gold:'#888', borderRadius:12, padding:'15px 18px', fontSize:17, fontWeight:packedBy===String(emp.id)?700:400, cursor:'pointer', textAlign:'left', fontFamily:'inherit', display:'flex', alignItems:'center', gap:12 }}>
            ...
            <button onClick={() => setPackedBy('packing_team')}
              style={{ background:packedBy==='packing_team'?'#3b82f622':'#141414', border: `2px solid ${packedBy==='packing_team'?'#3b82f6':'var(--border)'}`, color:packedBy==='packing_team'?'#3b82f6':'#555', borderRadius:12, padding:'15px 18px', fontSize:15, fontWeight:packedBy==='packing_team'?700:400, cursor:'pointer', textAlign:'left', fontFamily:'inherit', display:'flex', alignItems:'center', gap:12, marginTop:4 }}>
              <div style={{ width:38, height:38, borderRadius:'50%', background:packedBy==='packing_team'?'#3b82f6':'#2a2a2a', color: 'var(--text)', display:'flex', alignItems:'center', justifyContent:'center', fontSize:16, flexShrink:0 }}>👥</div>
