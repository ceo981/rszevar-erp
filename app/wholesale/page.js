...
const STATUS_CFG = {
  ...
  cancelled:  { label: 'Cancelled',  color: 'var(--text3)',    bg: '#55555522' },
};
    ...
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.8)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <div style={{ background: 'var(--bg-card)', border: `1px solid ${border}`, borderRadius: 12, padding: 28, width: 440, maxHeight: '90vh', overflowY: 'auto' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 13 }}>
          ...
          <div>
            <div style={{ fontSize: 11, color: 'var(--text3)', marginBottom: 5 }}>Notes</div>
...
function OrderModal({ buyers, onClose, onSave }) {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.8)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <div style={{ background: 'var(--bg-card)', border: `1px solid ${border}`, borderRadius: 12, padding: 28, width: 420 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 20 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 13 }}>
          <div>
            <div style={{ fontSize: 11, color: 'var(--text3)', marginBottom: 5 }}>Buyer</div>
          ...
          {selectedBuyer && <div style={{ fontSize: 12, color: 'var(--text3)', padding: '8px 12px', background: 'var(--bg-card)', borderRadius: 6 }}>
            Discount: {selectedBuyer.discount_pct}% · Credit Limit: {fmt(selectedBuyer.credit_limit)}
          </div>}
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
            <div>
              <div style={{ fontSize: 11, color: 'var(--text3)', marginBottom: 5 }}>Total Amount (Rs)</div>
              <input type="number" value={form.total_amount} onChange={e => setForm(f => ({...f, total_amount: e.target.value}))} placeholder="0"
                style={{ width: '100%', background: 'var(--bg-card)', border: `1px solid ${border}`, color: 'var(--text)', borderRadius: 7, padding: '9px 12px', fontSize: 13, boxSizing: 'border-box' }} />
            </div>
            <div>
              <div style={{ fontSize: 11, color: 'var(--text3)', marginBottom: 5 }}>Paid Amount (Rs)</div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
            <div>
              <div style={{ fontSize: 11, color: 'var(--text3)', marginBottom: 5 }}>Discount %</div>
              <input type="number" value={form.discount_pct} onChange={e => setForm(f => ({...f, discount_pct: e.target.value}))} placeholder="0"
                style={{ width: '100%', background: 'var(--bg-card)', border: `1px solid ${border}`, color: 'var(--text)', borderRadius: 7, padding: '9px 12px', fontSize: 13, boxSizing: 'border-box' }} />
            </div>
            <div>
              <div style={{ fontSize: 11, color: 'var(--text3)', marginBottom: 5 }}>Order Date</div>
              <input type="date" value={form.order_date} onChange={e => setForm(f => ({...f, order_date: e.target.value}))}
                style={{ width: '100%', background: 'var(--bg-card)', border: `1px solid ${border}`, color: 'var(--text)', borderRadius: 7, padding: '9px 12px', fontSize: 13 }} />
            </div>
          </div>
          <div>
            <div style={{ fontSize: 11, color: 'var(--text3)', marginBottom: 5 }}>Notes</div>
    ...
    <div style={{ fontFamily: 'Inter, sans-serif', color: 'var(--text)', padding: 24 }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 20 }}>
        <div>
          ...
          <p style={{ margin: '4px 0 0', fontSize: 13, color: 'var(--text3)' }}>Wholesale buyers aur orders manage karo</p>
        </div>
        <div style={{ display: 'flex', gap: 8 }}>
          <button onClick={() => { setEditBuyer(null); setShowBuyerModal(true); }}
            style={{ background: 'var(--bg-card)', border: `1px solid ${border}`, color: 'var(--text2)', borderRadius: 8, padding: '9px 16px', fontSize: 13, cursor: 'pointer' }}>
            + Add Buyer
          </button>
          <button onClick={() => setShowOrderModal(true)} disabled={buyers.length === 0}
            style={{ background: gold, color: 'var(--text)', border: 'none', borderRadius: 8, padding: '9px 18px', fontSize: 13, fontWeight: 700, cursor: buyers.length === 0 ? 'not-allowed' : 'pointer' }}>
            + New Order
          </button>
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: 12, marginBottom: 20 }}>
        {[
          { label: 'Total Buyers', value: buyers.length, color: 'var(--text)' },
      ...
      {/* Tabs */}
      <div style={{ display: 'flex', gap: 4, marginBottom: 16, background: 'var(--bg)', padding: 4, borderRadius: 9, width: 'fit-content' }}>
        {[['buyers', '👥 Buyers'], ['orders', '📋 Orders']].map(([id, label]) => (
          <button key={id} onClick={() => setTab(id)}
            style={{ background: tab === id ? '#1e1e1e' : 'transparent', border: `1px solid ${tab === id ? 'var(--border)' : 'transparent'}`, borderRadius: 7, padding: '7px 18px', fontSize: 13, color: tab === id ? gold : '#555', fontWeight: tab === id ? 600 : 400, cursor: 'pointer', fontFamily: 'inherit' }}>
            {label}
          </button>
        ))}
      </div>
                  ...
                  <div key={b.id} style={{ background: card, border: `1px solid ${border}`, borderRadius: 10, padding: '16px 18px' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 10 }}>
                      <div>
                        ...
                        {b.business_name && <div style={{ fontSize: 12, color: 'var(--text3)', marginTop: 2 }}>{b.business_name}</div>}
                      </div>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, fontSize: 12, marginBottom: 12 }}>
                      <div><div style={{ color: 'var(--text3)', marginBottom: 2 }}>Phone</div><div style={{ color: 'var(--text2)' }}>{b.phone || '—'}</div></div>
                      <div><div style={{ color: 'var(--text3)', marginBottom: 2 }}>City</div><div style={{ color: 'var(--text2)' }}>{b.city || '—'}</div></div>
                      <div><div style={{ color: 'var(--text3)', marginBottom: 2 }}>Discount</div><div style={{ color: gold, fontWeight: 600 }}>{b.discount_pct || 0}%</div></div>
                      <div><div style={{ color: 'var(--text3)', marginBottom: 2 }}>Credit Limit</div><div style={{ color: 'var(--text)' }}>{fmt(b.credit_limit)}</div></div>
                    </div>
                    {b.notes && <div style={{ fontSize: 11, color: 'var(--text3)', marginBottom: 10, padding: '6px 10px', background: 'var(--bg-card)', borderRadius: 6 }}>{b.notes}</div>}
                    <div style={{ display: 'flex', gap: 6 }}>
                      <button onClick={() => { setEditBuyer(b); setShowBuyerModal(true); }}
                        style={{ flex: 1, background: 'var(--bg-card)', border: `1px solid ${border}`, color: 'var(--text2)', borderRadius: 7, padding: '7px', fontSize: 12, cursor: 'pointer', fontFamily: 'inherit' }}>✏️ Edit</button>
                      <button onClick={() => deleteBuyer(b.id)}
                        style={{ background: 'var(--red-dim)', border: '1px solid #330000', color: '#ef4444', borderRadius: 7, padding: '7px 10px', fontSize: 12, cursor: 'pointer', fontFamily: 'inherit' }}>🗑</button>
                    </div>
                  </div>
                ))}
              </div>
                  ...
                  <thead>
                    <tr style={{ borderBottom: `1px solid ${border}` }}>
                      {['Buyer', 'Date', 'Total', 'Paid', 'Pending', 'Discount', 'Status', 'Actions'].map(h => (
                        <th key={h} style={{ padding: '11px 14px', textAlign: 'left', color: 'var(--text3)', fontWeight: 500, fontSize: 11, textTransform: 'uppercase', background: 'var(--bg)' }}>{h}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
                      ...
                      return (
                        <tr key={i} style={{ borderBottom: '1px solid #1a1a1a' }}>
                          <td style={{ padding: '11px 14px' }}>
                            <div style={{ color: 'var(--text)', fontWeight: 500 }}>{o.wholesale_buyers?.name || '—'}</div>
                            <div style={{ fontSize: 11, color: 'var(--text3)' }}>{o.wholesale_buyers?.city}</div>
                          </td>
                          <td style={{ padding: '11px 14px', color: 'var(--text2)', fontSize: 12 }}>{o.order_date}</td>
                          ...
                          <td style={{ padding: '11px 14px', color: 'var(--text2)' }}>{o.discount_pct || 0}%</td>
                          <td style={{ padding: '11px 14px' }}>
                          ...
                          <td style={{ padding: '11px 14px' }}>
                            <button onClick={() => { const p = prompt('Paid amount update karo:', o.paid_amount); if (p) updateOrder(o.id, o.status, p); }}
                              style={{ background: 'var(--bg-card)', border: `1px solid ${border}`, color: 'var(--text2)', borderRadius: 6, padding: '5px 10px', fontSize: 11, cursor: 'pointer', fontFamily: 'inherit' }}>
                              💰 Update
                            </button>
                          </td>
                        </tr>
