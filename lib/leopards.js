...
export async function getLeopardsCityId(cityName) {
  ...
  if (!match) {
    throw new Error(`City "${cityName}" Leopards mein nahi mili. Sahi city naam likhein (e.g. Karachi, Lahore, Multan)`);
  }
  return match.id;
}
...
export async function bookLeopardPacket({
  ...
  const shipperName = process.env.LEOPARDS_SHIPPER_NAME || 'self';
  const shipperPhone = process.env.LEOPARDS_SHIPPER_PHONE || 'self';
  const shipperAddress = process.env.LEOPARDS_SHIPPER_ADDRESS || 'self';
  const shipperEmail = process.env.LEOPARDS_SHIPPER_EMAIL || 'self';
  ...
  const payload = {
    ...
    return_address: shipperAddress !== 'self' ? shipperAddress : '',
  };
  ...
  return {
    ...
    tracking_url: data.track_number ? `https://leopardsfulfillment.leopardscourier.com/Track/Index?Cn=${data.track_number}` : null,
    raw: data,
  };
}

// ============================================================================
// ADDITION TO lib/leopards.js
// ============================================================================
// Copy the function below and PASTE it at the END of lib/leopards.js
// (after fetchLeopardsProof function, before the status mapping section)
//
// Purpose: Fetch Leopards courier status for specific tracking numbers.
// Used by the public /api/public/track endpoint for live tracking.
// ============================================================================

// ─── Fetch status for specific tracking numbers (for public tracker) ──────
// Same endpoint as fetchLeopardsStatuses, but queries by cn_numbers instead
// of date range. Max 50 CNs per call per Leopards API.
// Returns: array of packet objects (same shape as fetchLeopardsStatuses)
export async function fetchLeopardsStatusByTracking(cnNumbers) {
  const list = Array.isArray(cnNumbers) ? cnNumbers : [cnNumbers];
  const valid = list.filter(Boolean).map(String);
  if (valid.length === 0) return [];

  const data = await leopardsGet('getBookedPacketLastStatus', {
    cn_numbers: valid.slice(0, 50).join(','),
  });
