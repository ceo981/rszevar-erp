...
// ============================================================================
// WEIGHT — Aggregate order weight from Shopify line items
// ============================================================================
//
// Shopify ships `grams` per unit on every line_item (set from the variant's
// weight at the moment of purchase). ERP `order_items` table does NOT store
// weight, so we must always source it from `shopify_raw.line_items`.
//
// Use case: International orders me Abdul ko one-glance per order weight
// dikhna chahiye taa ke UDEX/courier rate slab decide ho sake (rate cards
// typically per-kg billing rakhte hain).
//
// Caveat: agar koi variant pe weight set nahi hai (grams=0), woh line zero
// contribute karegi. Total still shows, par incomplete ho sakta hai — user
// ko inventory me weight set karna chahiye.
// ============================================================================

/**
 * Compute total order weight in grams from raw Shopify line items.
 * Sums `grams × current_quantity` (or `quantity` fallback) across all active
 * line items.
 *
 * @param {Array} rawLineItems — `shopify_raw.line_items` (active or unfiltered)
 * @returns {number} total weight in grams
 */
export function computeOrderWeightGrams(rawLineItems) {
  if (!rawLineItems || rawLineItems.length === 0) return 0;
  return rawLineItems.reduce((sum, it) => {
    if (!isActiveRawLineItem(it)) return sum;
...
/**
 * Format a grams value as a human-readable weight string.
 *   < 1 kg  → "850 g"
 *   ≥ 1 kg  → "1.25 kg"     (2 decimals, trimmed trailing zeros)
 *   0 / invalid → "—"
 *
 * @param {number} grams
 * @returns {string}
...
export function formatWeight(grams) {
  ...
  if (!g || g <= 0 || Number.isNaN(g)) return '—';
  if (g >= 1000) {
    const kg = g / 1000;
    // Trim trailing zeros: 1.50 → "1.5 kg", 1.25 → "1.25 kg", 2.00 → "2 kg"
