...
export function transformProducts(shopifyProduct) {
  return (shopifyProduct.variants || []).map(variant => {
    ...
    return {
      ...
      image_url: (() => {
        // Variant-specific image (if assigned in Shopify)
        const variantImg = shopifyProduct.images?.find(
          img => img.variant_ids && img.variant_ids.includes(variant.id)
        );
        return variantImg?.src || shopifyProduct.image?.src || shopifyProduct.images?.[0]?.src || null;
      })(),
