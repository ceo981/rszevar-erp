...
export async function fetchAllProducts() {
  ...
  let pageCount = 0;
  const MAX_PAGES = 40; // safety: 40 pages × 50 = 2000 products max

  while (hasNextPage && pageCount < MAX_PAGES) {
    const result = await shopifyGraphQL(PRODUCTS_GRAPHQL_QUERY, { cursor });
    ...
    // Rate limit protection: 500ms delay between pages
    if (hasNextPage) {
