...
// Phase A (Apr 28 2026): transformProducts now captures description_html,
//          tags, handle, images_data, shopify_status — needed for full
//          product editing inside ERP. Schema columns added in same phase.
// ============================================================================
...
// ============================================================================
// Payment method derivation (May 22 2026 — fix for "all orders show as COD")
// ----------------------------------------------------------------------------
// Pehle line 237 sirf `gateway` field dekhti thi aur 'manual' (jise hum
// "Bank Transfer" call karte hain) ko bhi 'COD' bana deti thi. Iska natija:
//   - Bank Transfer orders galti se COD totals mein chale jaate the
//   - Delivery pe auto-mark paid bhi ho jaate the (jo ke ghalat — pehle
//     payment confirm honi chahiye)
//   - Order page pe "Payment method: COD" hi dikhta tha sab ke liye
//
// Ab proper 3-way distinction (user ke 3 Shopify methods ke saath align):
//   - 'COD'           → courier customer se cash collect karega on delivery
//   - 'Bank Transfer' → manual gateway, customer ne bank transfer kiya hai
//   - 'Card'          → Shopify Payments / Stripe / online card paid
//   - <other>         → koi aur custom gateway naam ho to pass-through
//
// Downstream COD-only logic (loadsheet COD amount, scan-dispatch COD,
// auto-mark-paid on delivery, accounts COD revenue) ab automatically
// Bank Transfer & Card orders ko sahi se exclude karega.
//
// Data source priority:
//   1. payment_gateway_names[0]  — Shopify ka authoritative display name
//      (yahan se "Bank Transfer" jaisa user-named gateway aata hai)
//   2. gateway                    — legacy lowercase field, fallback
//
// Note: Mark-paid endpoint UI se Bank Alfalah / Easypaisa / JazzCash / Cash
// etc receipt methods bhi yahi payment_method column mein overwrite kar
// deta hai jab staff manually paid mark karta hai — yeh hybrid behaviour
// pehle bhi tha aur reporting ke liye useful hai (unpaid orders me Shopify
// gateway dikhta hai, paid orders me actual receipt method).
function derivePaymentMethod(shopifyOrder) {
  ...
  if (!primary) return 'COD'; // safe fallback — pehle wala default behaviour

  const lower = primary.toLowerCase();

  // Cash on Delivery — multiple format variants ("COD", "Cash on Delivery (COD)", etc.)
  if (lower === 'cod' || lower.includes('cash on delivery')) {
    return 'COD';
  }

  // Manual gateway (Shopify ka default "manual" type) = bank transfer in RS ZEVAR's
  // setup (per Abdul May 22 2026). "Bank" word ho to bhi yahi bucket.
  if (lower === 'manual' || lower.includes('bank')) {
    return 'Bank Transfer';
  }

  // Card processors → unified "Card" label
  if (
  ...
  // Else preserve the Shopify-side name as-is (e.g. agar future mein koi
  // naya gateway add ho jaye — hum usko swallow nahi karenge)
  return primary;
}
...
export async function transformOrder(shopifyOrder) {
  ...
  const base = {
    ...
    payment_method: derivePaymentMethod(shopifyOrder),
    status,
    payment_status,
    tags,
    is_wholesale,
...
// ============================================================================
// buildImageLookupMaps — shared helper for every transformLineItems caller
// ----------------------------------------------------------------------------
// FIX May 2026 — Variant-aware image lookup was added to transformLineItems
// on May 8 2026, but the 5 callers that actually invoke it (sync, backfill,
// webhook pipeline, orders/edited webhook, edit-commit) were only building
// `skuImageMap` and never passing `variantImageMap`. Result: variant_id-based
// fallback was dead code, ALL writes degraded to SKU fallback, and orders
// like ZEVAR-119546 (White variant of MS1142) picked up whatever color came
// first in the SKU map.
//
// Fix is to consolidate the map-building logic here so every caller becomes
// a one-liner and the maps are always populated consistently. Single DB
// query selects all three columns at once.
//
// Returns:
//   { skuImageMap, variantImageMap }
//     skuImageMap     : { [sku]: image_url }       — first-hit-wins (legacy)
//     variantImageMap : { [variant_id]: image_url } — exact match (new)
//
// Safe defaults: if the query fails, both maps return empty objects so
// transformLineItems falls through to `line_item.image?.src` or `null`.
// ============================================================================
export async function buildImageLookupMaps(supabase) {
  const skuImageMap = {};
  const variantImageMap = {};
  try {
    const { data: rows } = await supabase
      .from('products')
      .select('sku, shopify_variant_id, image_url')
      .not('image_url', 'is', null);
    for (const p of rows || []) {
      if (!p.image_url) continue;
      // variant_id map — exact match, always set (variant_id unique by design)
      if (p.shopify_variant_id) {
        variantImageMap[p.shopify_variant_id] = p.image_url;
      }
      // sku map — first hit wins (legacy semantics preserved for fallback)
      if (p.sku && !skuImageMap[p.sku]) {
        skuImageMap[p.sku] = p.image_url;
      }
    }
  } catch {
    // Silent fallback — empty maps; transformLineItems uses line_item.image?.src
  }
  return { skuImageMap, variantImageMap };
}

export function transformLineItems(shopifyOrder, skuImageMap = {}, variantImageMap = {}) {
  return (shopifyOrder.line_items || [])
    ...
    .map(item => {
      // FIX May 8 2026 — variant_id-aware image fallback.
      // Pehle skuImageMap fallback use hota tha. Lekin RS ZEVAR ke jewelry
      // products mein typically Golden/Silver variants ka SKU same hota hai
      // (parent SKU shared). Is wajah se skuImageMap ambiguous tha — first-hit
      // wins, random variant ki image fallback mein chali jati thi.
      // Example bug: ZEVAR-119241 mein Golden variant (SKU AD083) ki jaga
      // Silver image dikhi.
      // Ab priority:
      //   1) Shopify line_item ka image (variant-specific, sahi color)
      //   2) variantImageMap[variant_id] — products table se variant_id ke
      //      hisab se exact image (variant_id always unique, never collides)
      //   3) skuImageMap[sku] — purana fallback, jab variant_id na ho
      //   4) null
      //
      // FIX May 2026 — Cleaned up dead `|| null` in the middle of the
      // expression. Logically harmless (`null` is falsy in `||`) but caller
      // sites were ALSO never passing variantImageMap, making priority #2
      // dead code. buildImageLookupMaps() ab dono maps ek saath build karta
      // hai aur har caller use karta hai — see callers list in helper docs.
      const variantId = item.variant_id ? String(item.variant_id) : null;
      const imageUrl =
        item.image?.src
        || (variantId && variantImageMap[variantId])
        || (item.sku && skuImageMap[item.sku])
        || null;
      ...
      return {
        ...
        // FIX May 8 2026 — Capture shopify_variant_id for accurate lookups.
        // Going forward, all DB joins (products → order_items image) prefer
        // variant_id over SKU (variant_id unique, SKU may collide).
...
// Phase A (Apr 2026): Now also captures product-level fields needed for
// in-ERP product editing — description_html, tags, handle, images_data
// (with alt texts), shopify_status. These are duplicated across all variants
// of same product (read-efficient; mirrors how Shopify's REST returns them).
export function transformProducts(shopifyProduct) {
  // ── Product-level fields (same value for every variant of this product) ──
  const productLevel = {
    description_html: shopifyProduct.body_html || null,
    tags: shopifyProduct.tags
      ? String(shopifyProduct.tags).split(',').map(t => t.trim()).filter(Boolean)
      : [],
    handle: shopifyProduct.handle || null,
    images_data: (shopifyProduct.images || []).map(img => ({
      id: String(img.id),
      src: img.src,
      alt: img.alt || null,
      ...
      variant_ids: (img.variant_ids || []).map(v => String(v)),
    })),
    shopify_status: shopifyProduct.status || null,
  };
  ...
  return (shopifyProduct.variants || []).map(variant => {
    ...
    // FIX Apr 30 2026 — Weight ko grams mein normalize karo.
    // Pehle directly `parseFloat(variant.weight)` use hota tha, jo galat hota
    // hai jab `weight_unit` 'kg' / 'lb' / 'oz' ho. ERP saari jagah grams expect
    // karta hai (courier slips, packing list, etc.).
    //
    // Source priority:
    //   1) `variant.grams` — Shopify always returns grams here, exact integer
    //   2) `variant.weight` + `variant.weight_unit` — convert to grams
    let weightInGrams = 0;
    if (variant.grams !== undefined && variant.grams !== null) {
      weightInGrams = Number(variant.grams) || 0;
    } else if (variant.weight !== undefined && variant.weight !== null) {
      const w = parseFloat(variant.weight) || 0;
      const unit = String(variant.weight_unit || 'g').toLowerCase();
      if (unit === 'kg')      weightInGrams = w * 1000;
      else if (unit === 'lb') weightInGrams = w * 453.592;
      else if (unit === 'oz') weightInGrams = w * 28.3495;
      else                    weightInGrams = w;       // 'g' or unknown — assume grams
    }

    // FIX May 8 2026 — Variant image lookup: forward `variant.image_id` lookup
    // first (most reliable), then reverse `image.variant_ids` lookup, then
    // product-level fallbacks.
    //
    // Background: Shopify maintains both sides of the variant↔image link in
    // theory, but in practice the reverse mapping (`image.variant_ids`) can
    // be missing/empty even when `variant.image_id` is set — happens with CSV
    // imports, certain API write paths, and after image reorders. Using the
    // forward link first eliminates the silent silver-instead-of-golden bug.
    let variantImageUrl = null;
    if (variant.image_id && Array.isArray(shopifyProduct.images)) {
      const directImg = shopifyProduct.images.find(img => String(img.id) === String(variant.image_id));
      if (directImg?.src) variantImageUrl = directImg.src;
    }
    // Reverse lookup as defense-in-depth
    if (!variantImageUrl && Array.isArray(shopifyProduct.images)) {
      const reverseImg = shopifyProduct.images.find(img => img.variant_ids?.includes(variant.id));
      if (reverseImg?.src) variantImageUrl = reverseImg.src;
    }
    const finalImageUrl = variantImageUrl
    ...
    return {
      ...
      // ── New product-level fields (Phase A) ──
      ...productLevel,
    };
  });
}
...
// May 2026 — Add TTL to module-level location cache. Without TTL the cached
// id sticks for the entire Lambda warm lifetime (could be hours), and if
// Abdul ever changes the active Shopify location (rare but possible), the
// next fulfillment call would fail until the function cold-starts. 10 min
// TTL gives near-zero overhead in steady state with a clean recovery path.
let cachedLocationId = null;
let cachedLocationAt = 0;
const LOCATION_TTL_MS = 10 * 60 * 1000;

export async function getShopifyLocationId() {
  if (cachedLocationId && (Date.now() - cachedLocationAt) < LOCATION_TTL_MS) {
...
export async function createShopifyFulfillment(shopifyOrderId, trackingNumber, courier, trackingUrl, opts = {}) {
  // FIX Apr 2026 — tracking_number is now OPTIONAL.
  // Use case: manual fulfill for walk-in / wholesale / pickup orders that
  // don't have a tracking number. Existing callers (dispatch route) still
  // pass a tracking number, so this is a non-breaking change.
  if (!shopifyOrderId) throw new Error('shopifyOrderId required');

  // Allow caller to control customer notification (default: true to preserve
  // historical behavior of dispatch route).
  const notifyCustomer = opts.notify_customer !== false;
  ...
  // Include tracking_info only when a tracking number is provided.
  // For tracking-less fulfillment (walk-in / wholesale / pickup), Shopify
  // marks the line items as fulfilled without sending a shipping email.
  if (trackingNumber) {
    fulfillmentBody.tracking_info = {
      number: trackingNumber,
      company: mapCourierToShopifyCarrier(courier),
      url: trackingUrl || `https://lcs.appsbymoose.com/track/${trackingNumber}`,
    };
  }

  const data = await shopifyFetch('fulfillments.json', {}, {
    method: 'POST',
    body: { fulfillment: fulfillmentBody },
  });

  return data.fulfillment;
}

// Cancel a Shopify fulfillment (reverses createShopifyFulfillment).
// Use case: tracking add ho gayi but baad mein realize hua ke courier change
// karna hai, ya double-booking ho gayi, ya order edit hua. ERP se cancel
// fulfillment kar do, tracking hat jayegi, items wapas "unfulfilled" state mein.
//
// Note: Shopify ke server-side webhook bhi fire karega (fulfillments/cancelled)
// jo dobara same cleanup karega. Idempotent design — safe to call.
export async function cancelShopifyFulfillment(fulfillmentId) {
  if (!fulfillmentId) throw new Error('fulfillmentId required');
  const data = await shopifyFetch(`fulfillments/${fulfillmentId}/cancel.json`, {}, {
    method: 'POST',
  });
  return data.fulfillment;
}
...
function mapShopifyStatus(order, tags = [], rules = {}) {
  ...
  // Apr 2026 — Walk-in auto-deliver / auto-paid rules REMOVED.
  // Pehle: agar order par 'walkin' tag hota tha to system automatically status
  // 'delivered' aur payment 'paid' kar deta tha. Iska problem yeh tha ke walk-in
  // tag ka matlab sirf "yeh walk-in customer hai" hai — koi auto-action nahi
  // hona chahiye. Ab walk-in/international/wholesale sab sirf INFORMATIONAL
  // labels hain. Saara flow same — manager confirm karega, dispatcher fulfill +
  // dispatch karega. Tag sirf badge dikhata hai order page par.
  ...
  // NOTE: Walk-in / International / Wholesale tags ka koi auto-action nahi.
  // Yeh sirf informational hain. Saara flow normal — same as retail.
