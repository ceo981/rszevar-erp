...
import { derivePaymentMethod } from './payment-method';
...
function normalizeCourierName(name) {
  ...
  // Trax (public API brand "Sonic"). Shopify tracking_company shows "Trax".
  if (lower.includes('trax') || lower.includes('sonic')) return 'Trax';
...
const KNOWN_COURIERS_FOR_FALLBACK = ['PostEx', 'Leopards', 'Kangaroo', 'Trax', 'TCS', 'M&P'];
...
// NOTE — Payment method detection logic ab `lib/payment-method.js` mein
// hai (shared between server transform path aur client display fallback).
// Webhook lock (lib/shopify-webhook.js) ERP-set payment_method ko preserve
// karta hai jab Shopify side empty gateway bhejta hai (ERP-created drafts).
...
function mapCourierToShopifyCarrier(courier) {
  const map = { 'PostEx': 'PostEx', 'Leopards': 'Leopards Courier', 'Kangaroo': 'Kangaroo Logistics', 'Trax': 'Trax' };
...
export async function createShopifyFulfillment(shopifyOrderId, trackingNumber, courier, trackingUrl, opts = {}) {
  ...
  if (trackingNumber) {
    fulfillmentBody.tracking_info = {
      ...
      url: trackingUrl || `https://leopardsfulfillment.leopardscourier.com/Track/Index?Cn=${trackingNumber}`,
    };
  }
