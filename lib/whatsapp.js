 ...
 */
export async function sendOrderDispatched({ phone, customer_name, order_number, courier, tracking_number }) {
  const firstName = (customer_name || 'Customer').split(' ')[0];
 ...
 */
export async function sendOrderConfirmInteractive({ phone, customer_name, order_number, total_amount }) {
  const firstName = (customer_name || 'Customer').split(' ')[0];
  const amount = total_amount ? Number(total_amount).toLocaleString() : '0';
