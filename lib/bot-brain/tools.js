...
// ─── Collections lookup (real website collections, not memory categories) ────
// Powers the "View more" link under product cards. Matches the customer's query
// against the live `collections` table (handle, title, products_count synced
// from Shopify) and returns the best collection { title, url }.
let _collCache = null;
let _collAt = 0;
const COLL_TTL_MS = 5 * 60 * 1000;

async function getCollectionsCached(db) {
  if (_collCache && Date.now() - _collAt < COLL_TTL_MS) return _collCache;
  try {
    const { data } = await db
      .from('collections')
      ...
      .limit(120);
    _collCache = (data || []).filter((c) => c.handle && c.title);
    _collAt = Date.now();
  } catch {
    _collCache = _collCache || [];
  }
  return _collCache;
}

async function findCollectionForQuery(db, query) {
  const q = String(query || '').toLowerCase();
  const terms = q.split(/[^a-z0-9]+/).filter((t) => t.length >= 3);
  if (terms.length === 0) return null;

  let cols;
  try { cols = await getCollectionsCached(db); } catch { return null; }
  ...
  for (const c of cols) {
    ...
    let s = 0;
    for (const term of terms) if (title.includes(term)) s++;
    // small boost if the collection title is fully contained in the query
    if (q.includes(title)) s += 1;
    if (s > bestScore || (s === bestScore && best && (c.products_count || 0) > (best.products_count || 0))) {
      best = c;
      bestScore = s;
    }
  }
  if (!best || bestScore === 0) return null;
  return { title: best.title, url: `${SITE}/collections/${best.handle}` };
}
...
export async function searchProducts(query, db) {
  ...
  const collection = await findCollectionForQuery(db, raw);
  return { count: products.length, results: products, collection };
}
...
export async function getBestSellers(db, limit) {
  ...
  const collection = await findCollectionForQuery(db, 'best seller');
  return { count: products.length, results: products, collection };
}
