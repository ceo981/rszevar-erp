...
export async function maybeAutoReply({ conversationId, fromPhone, msgType, bodyText }) {
  try {
    ...
    // 3) Human takeover: if the last outbound msg was sent by a person recently, stay quiet
    const { data: lastOut } = await db
      .from('whatsapp_messages')
      .select('sent_by_system, created_at')
      .eq('conversation_id', conversationId)
      .eq('direction', 'out')
      .order('created_at', { ascending: false })
      .limit(1)
      .maybeSingle();
    if (lastOut && lastOut.sent_by_system === false) {
      const ageH = (Date.now() - new Date(lastOut.created_at).getTime()) / 3600000;
      if (ageH < HUMAN_TAKEOVER_HOURS) return;
    }
