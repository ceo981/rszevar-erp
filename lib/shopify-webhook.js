...
import { transformOrder, transformLineItems, buildImageLookupMaps } from './shopify';
...
export async function handleOrderWebhook(request, { topic, insertLineItems = false }) {
  ...
  // May 5 2026 — Added is_credit_order to detect credit/udhaar orders so we
  // can guard them against Shopify's COD-on-fulfillment auto-paid behavior.
  const { data: existing } = await supabase
    .from('orders')
    .select('id, status, payment_status, payment_method, confirmed_at, is_credit_order')
  ...
  if (existing) {
    if (isShopifyCancellation) {
      ...
      // ── CREDIT ORDER GUARD (May 5 2026) ──────────────────────────────────
      // is_credit_order=true orders ka payment_status ERP ke credit module se
      // control hota hai (customer_payments + payment_allocations table).
      // Shopify ke webhooks ko isme hath NAHI lagana chahiye — warna COD-on-
      // fulfillment auto-paid behavior credit orders ko galti se paid mark
      // kar deta hai. (Bug fix: ZEVAR-118627 case, May 4 2026 — order khaate
      // se ghaib ho gaya tha kyunki Shopify ne `orders/paid` webhook bheja
      // delivery ke 5 second baad.)
      //
      // Net effect: Once is_credit_order=true, Shopify cannot flip the order's
      // payment_status. ERP credits module is the source of truth.
      if (existing.is_credit_order) {
        if ('payment_status' in orderData) {
          console.log(`[webhook:${topic}] ⚠ Credit order ${orderData.order_number || existing.id} — ignoring Shopify payment_status='${orderData.payment_status}' (ERP credit module is source of truth)`);
          delete orderData.payment_status;
        }
      }

      // ── PAYMENT METHOD LOCK (May 22 2026) ────────────────────────────────
      // ERP-side payment_method ko protect karte hain jab Shopify ke paas
      // gateway info hi nahi hai. Yeh case mainly ERP-created orders ke liye
      // hota hai: Draft Order complete with payment_pending=true se bana
      // order — Shopify gateway empty, payment_gateway_names empty array.
      // Aise mein derivePaymentMethod default 'COD' return karega, jo wrongly
      // ERP-side 'Bank Transfer'/'Card' selection ko overwrite kar dega.
      //
      // Logic: agar Shopify ke paas koi gateway info nahi hai AND existing
      // row mein already payment_method set hai → naya value drop kar do
      // (ERP-side authoritative remains).
      //
      // Mark-paid receipt methods (Cash / Bank Alfalah / JazzCash / etc.) bhi
      // isi tarah protected hain — Shopify ke baad ke webhooks unhe nahi
      // chhede ge jab tak Shopify side pe explicit gateway info na ho.
      //
      // May 30 2026 FIX — 'manual' gateway AMBIGUOUS hai. Shopify draft order
      // ko payment_pending=true se complete karne par gateway='manual' set
      // hota hai chahe user ne COD pick kiya ho ya Bank Transfer — yaani
      // 'manual' se hum COD vs Bank Transfer distinguish NAHI kar sakte. Pehla
      // code sirf empty gateway ko "no info" maanta tha, magar 'manual' ko
      // "has info" samajh kar lock skip kar deta tha → ERP ki 'COD' value
      // 'Bank Transfer' se overwrite ho jati thi (Bug: COD order → Bank
      // Transfer dikhta tha).
      //
      // Ab sirf NON-manual, non-empty gateway ko "reliable" maante hain. Jab
      // gateway manual/empty ho AND ERP ke paas already payment_method ho
      // (create page se user choice, ya pehle derive hui reliable value, ya
      // mark-paid receipt method) → us ERP value ko preserve karte hain.
      const gatewaySignals = [
        ...
        .filter(Boolean);
      const hasReliableGateway = gatewaySignals.some(g => g !== 'manual');
      if (!hasReliableGateway && existing.payment_method && 'payment_method' in orderData) {
        if (orderData.payment_method !== existing.payment_method) {
          console.log(`[webhook:${topic}] 🔒 Preserving ERP payment_method='${existing.payment_method}' (Shopify gateway manual/empty & ambiguous; new derive would be '${orderData.payment_method}')`);
        }
  ...
  if (shouldSyncLineItems) {
    // FIX May 2026 — Build BOTH sku and variant_id image maps in one query.
    // Pehle sirf skuImageMap banta tha, jiski wajah se variant_id-based image
    // fallback (transformLineItems mein) dead code tha aur same-SKU variants
    // ki images mix ho jati thi. See lib/shopify.js#buildImageLookupMaps.
    ...
    const items = transformLineItems(shopifyOrder, skuImageMap, variantImageMap).map(i => ({ ...i, order_id: orderId }));
