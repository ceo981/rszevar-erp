...
export async function handleOrderWebhook(request, { topic, insertLineItems = false }) {
  ...
  if (existing) {
    if (isShopifyCancellation) {
      ...
      const shopifyHasGatewayInfo =
        !!shopifyOrder.gateway ||
        (Array.isArray(shopifyOrder.payment_gateway_names) && shopifyOrder.payment_gateway_names.length > 0);
      if (!shopifyHasGatewayInfo && existing.payment_method && 'payment_method' in orderData) {
        if (orderData.payment_method !== existing.payment_method) {
          console.log(`[webhook:${topic}] 🔒 Preserving ERP payment_method='${existing.payment_method}' (Shopify gateway empty; new derive would be '${orderData.payment_method}')`);
        }
