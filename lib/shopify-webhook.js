 ...
 *
 * PROTOCOL FIX (Apr 2026): Shopify webhook ab silently office status ko pre-
...
export async function handleOrderWebhook(request, { topic, insertLineItems = false }) {
  ...
  // 3b. Validate — sirf real orders process karo
  // Draft orders, abandoned checkouts, test orders skip karo
  ...
  let protocolViolation = null; // Store for logging later

  if (existing) {
    if (lockedStatuses.includes(existing.status)) {
      delete orderData.status;
      // Also strip confirmed_at so existing timestamp stays intact
    ...
    // If existing already has confirmed_at, preserve it (don't overwrite with
    // a fresh timestamp even if status is flowing through as 'confirmed')
    if (existing.confirmed_at && orderData.confirmed_at) {
      delete orderData.confirmed_at;
    }

    // -----------------------------------------------------------------------
    // PROTOCOL GUARD: If Shopify sends tracking (courier booked externally)
    // but order is still in pre-dispatch zone (pending/confirmed/on_packing),
    // DO NOT auto-promote the office status. Just save tracking info and log
    // exception. CEO will see this in activity log.
    // -----------------------------------------------------------------------
    ...
    if (hasActiveTracking && PRE_DISPATCH_STATUSES.includes(existing.status)) {
      // Don't allow any automatic status change while in pre-dispatch zone
      delete orderData.status;
      delete orderData.confirmed_at;
      protocolViolation = {
  ...
  // 7b. Log protocol violation (best-effort)
  if (protocolViolation && orderId) {
    try {
      await supabase.from('order_activity_log').insert({
        order_id: orderId,
        action: 'protocol_violation:shopify_tracking_ahead_of_office',
        notes: `Shopify fulfillment tracking aayi magar office status abhi "${protocolViolation.office_status}" pe hai. ` +
               `Office status unchanged — staff ne ERP flow skip kiya. ` +
               `Confirm/Packed buttons click nahi hue.`,
  ...
  if (insertLineItems && orderId && wasNew) {
    // Build SKU → image_url map from products table (so new orders get images immediately)
    let skuImageMap = {};
    try {
  ...
  // 8b. WhatsApp confirmation — insertLineItems (orders/create) pe bhejo
  // wasNew check nahi — whatsapp_logs se duplicate check karo
  if (insertLineItems && orderId) {
    ...
    if (customerPhone) {
      try {
        // Already sent check — duplicate bhejne se bachao
