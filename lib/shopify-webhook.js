...
export async function handleOrderWebhook(request, { topic, insertLineItems = false }) {
  ...
  if (existing) {
    ...
    // Special: confirmed → on_packing when Shopify tracking is added
    // Rule: courier booking Shopify pe hoti hai → tracking milti hai → on_packing
    if (existing.status === 'confirmed') {
      ...
      if (hasTracking) {
        orderData.status = 'on_packing'; // override — confirmed se on_packing
      }
    }
  }
