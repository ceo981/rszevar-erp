...
// v2.1 — supports preloadedCandidates option for batch performance
...
// Fetch all active+in-stock product rows (deduped to one per parent).
// Used both by single-product mode and by batch mode (which calls this once and passes down).
export async function loadCandidatePool(supabase) {
  const rows = [];
  const PAGE = 1000;
  let off = 0;
  while (off < 6000) {
    ...
    if (error) throw new Error(`Candidate pool fetch failed: ${error.message}`);
...
// Options:
//   preloadedPool: Array — full candidate pool (skip DB fetch when provided by batch)
export async function generateRelatedProductsForOne(supabase, shopify_product_id, options = {}) {
  ...
  // ── Step 2: Get candidate pool (preloaded in batch mode, else fetch now) ──
  let pool = options.preloadedPool;
  if (!pool) {
    pool = await loadCandidatePool(supabase);
  }

  // Exclude self + apply price band
  const candPriceMin = (targetRow.selling_price || 0) * 0.4;
  const candPriceMax = (targetRow.selling_price || 0) * 1.6;
  let allCandidates = pool.filter(c => c.shopify_product_id !== pid);
  ...
  // In batch mode, use pool to enrich without extra DB call
  const pickIds = picks.map(p => p.shopify_product_id);
  let snapshots = {};
  if (pickIds.length > 0) {
    if (options.preloadedPool) {
      // Use pool (no image_url in pool — need lightweight fetch for images)
