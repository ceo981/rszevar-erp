...
// v2.2 — adds retry/backoff for rate limits + 429 handling
...
// Load candidate pool (used by both single mode and batch)
...
// Claude API call with retry + exponential backoff for rate limits
async function callClaudeWithRetry(client, params, maxRetries = 3) {
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    try {
      ...
      const isOverloaded = status === 529 || status === 503 || msg.includes('overloaded');
      const isRetryable = isRateLimit || isOverloaded;

      if (!isRetryable || attempt === maxRetries) throw e;

      // Exponential backoff: 3s, 6s, 12s + random jitter up to 1s
      const baseDelay = 3000 * Math.pow(2, attempt);
      const jitter = Math.random() * 1000;
      const delay = baseDelay + jitter;

      console.log(`[related-products] Rate limit hit, retry ${attempt + 1}/${maxRetries} after ${Math.round(delay)}ms`);
...
export async function generateRelatedProductsForOne(supabase, shopify_product_id, options = {}) {
  ...
  // Step 2: Get candidate pool (preloaded in batch mode)
  let pool = options.preloadedPool;
  if (!pool) pool = await loadCandidatePool(supabase);

  // Exclude self + price band
  ...
  if (candidates.length === 0) {
    ...
    // Higher built-in maxRetries + custom retry wrapper for robustness
